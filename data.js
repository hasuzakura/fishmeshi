const FISH_LIST = [
  { name: "アジ",   slug: "aji",     photo: "https://upload.wikimedia.org/wikipedia/commons/d/d9/MaAji.jpg" },
  { name: "サバ",   slug: "saba",    photo: "https://upload.wikimedia.org/wikipedia/commons/thumb/2/24/Scomber_japonicus_San_Diego.jpg/960px-Scomber_japonicus_San_Diego.jpg" },
  { name: "イワシ", slug: "iwashi",  photo: "https://upload.wikimedia.org/wikipedia/commons/thumb/b/bf/Sardinops_melanostictus.jpg/960px-Sardinops_melanostictus.jpg" },
  { name: "タイ",   slug: "tai",     photo: "https://upload.wikimedia.org/wikipedia/commons/thumb/b/b9/Pagrus_major_Red_seabream_ja01.jpg/960px-Pagrus_major_Red_seabream_ja01.jpg" },
  { name: "スズキ", slug: "suzuki",  photo: "https://upload.wikimedia.org/wikipedia/commons/5/55/Suzuki_%28Lateolabrax%29.jpg" },
  { name: "ブリ",   slug: "buri",    photo: "https://upload.wikimedia.org/wikipedia/commons/c/c0/Buri.jpg" },
  { name: "カサゴ", slug: "kasago",  photo: "https://upload.wikimedia.org/wikipedia/commons/thumb/a/aa/Kasago.JPG/960px-Kasago.JPG" },
  { name: "メバル", slug: "mebaru",  photo: "https://upload.wikimedia.org/wikipedia/commons/7/78/Mebaru.jpg" },
  { name: "ハマチ", slug: "hamachi", photo: "https://upload.wikimedia.org/wikipedia/commons/a/a9/Seriola_quinqueradiata_%28200810%29.jpg" },
  { name: "キス",   slug: "kisu",    photo: "https://upload.wikimedia.org/wikipedia/commons/thumb/9/9f/Sillago_japonica_Kagoshima.jpg/960px-Sillago_japonica_Kagoshima.jpg" },
  { name: "ニジマス", slug: "nijimasu", photo: "https://upload.wikimedia.org/wikipedia/commons/thumb/c/c6/Rainbow_Trout_%28Oncorhynchus_mykiss%29_Gavins_Point.jpg/960px-Rainbow_Trout_%28Oncorhynchus_mykiss%29_Gavins_Point.jpg" },
];

const PLACES = [
  { name: "家のキッチン", desc: "調理設備が充実している", photo: "https://images.unsplash.com/photo-1734018959097-1fd0696a1298?w=800&q=80&fit=crop&auto=format" },
  { name: "キャンプ場",   desc: "アウトドアで調理",       photo: "https://images.unsplash.com/photo-1692313532975-513a383c48f7?w=800&q=80&fit=crop&auto=format" },
  { name: "釣り場その場", desc: "釣り場で即料理",         photo: "https://images.unsplash.com/photo-1760118947136-619c8f74418a?w=800&q=80&fit=crop&auto=format" },
];

const TOOLS = [
  { name: "なんでもある",   desc: "フライパン・包丁・オーブンなど全部OK", photo: "https://images.unsplash.com/photo-1584990347193-6bebebfeaeee?w=200&q=80&fit=crop&auto=format" },
  { name: "フライパン",     desc: "フライパンと火があれば",               photo: "https://images.unsplash.com/photo-1581622558638-818128465982?w=200&q=80&fit=crop&auto=format" },
  { name: "グリル・網",     desc: "炭火・ガス火・グリルで焼く",           photo: "https://images.unsplash.com/photo-1653932618064-8ff69761bf29?w=200&q=80&fit=crop&auto=format" },
  { name: "包丁まな板あり", desc: "刺身など生食系もOK",                   photo: "https://images.unsplash.com/photo-1690983322029-eee73c0afa14?w=200&q=80&fit=crop&auto=format" },
];

const SEASONINGS_LIST = [
  { name: "塩",     emoji: "🧂" },
  { name: "醤油",   emoji: "🍶" },
  { name: "みりん", emoji: "🍶" },
  { name: "料理酒", emoji: "🍶" },
  { name: "砂糖",   emoji: "🍬" },
  { name: "味噌",   emoji: "🫙" },
  { name: "酢",     emoji: "🫙" },
  { name: "バター", emoji: "🧈" },
  { name: "サラダ油", emoji: "🫙" },
  { name: "ごま油", emoji: "🫙" },
  { name: "にんにく", emoji: "🧄" },
  { name: "生姜",   emoji: "🌿" },
  { name: "薄力粉", emoji: "🌾" },
  { name: "片栗粉", emoji: "🌾" },
  { name: "ポン酢", emoji: "🍋" },
  { name: "わさび", emoji: "🌿" },
];

const AMAZON_BASE = "https://www.amazon.co.jp/s?k=";
const AMAZON_TAG = "fishmeshi-22";

// Google AdSense。true にすると全ページに広告タグが入り、
// プライバシーポリシーの「広告について」がCookie・第三者配信の説明を含む文面に切り替わる。
// 広告タグを設置した時点でGoogleが訪問者の情報を取得しうるため、
// 「承認後」ではなく「タグを設置した時点」で true にすること。
const ADSENSE_ENABLED = true;
const ADSENSE_CLIENT = "ca-pub-7252742932766480";

const RECIPES = {
  "アジ": [
    {
      slug: "sashimi",
      name: "アジの刺身",
      places: ["家のキッチン", "釣り場その場"],
      tools: ["包丁まな板あり", "なんでもある"],
      seasonings: ["醤油", "わさび"],
      servings: "2人分", time: "15分",
      ingredients: ["アジ 2尾", "醤油 適量", "わさび 適量", "大葉・大根おろし（あれば）"],
      steps: [
        "アジのうろこ・エラ・内臓を取り除き、水で洗って水気を拭く。",
        "頭を落とし、三枚おろしにする。腹骨を削ぎ取る。",
        "皮を引き、食べやすい厚さ（約7mm）に切る。",
        "器に盛り、わさびと醤油を添えて完成。",
      ],
    },
    {
      slug: "fry",
      name: "アジフライ",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["塩", "薄力粉", "サラダ油"],
      servings: "2人分", time: "25分",
      ingredients: ["アジ 2尾（三枚おろし）", "塩こしょう 少々", "薄力粉 大さじ3", "溶き卵 1個分", "パン粉 適量", "サラダ油 揚げ油"],
      steps: [
        "三枚おろしにしたアジに塩こしょうをして5分置く。",
        "薄力粉→溶き卵→パン粉の順にまぶす。",
        "180℃のサラダ油で3〜4分、こんがりきつね色になるまで揚げる。",
        "油を切り、タルタルソースやウスターソースを添えて完成。",
      ],
    },
    {
      slug: "namerou",
      name: "アジのなめろう",
      places: ["家のキッチン", "釣り場その場", "キャンプ場"],
      tools: ["包丁まな板あり", "なんでもある"],
      seasonings: ["味噌", "生姜"],
      servings: "2人分", time: "10分",
      ingredients: ["アジ 2尾（三枚おろし）", "味噌 大さじ1", "生姜 小さじ1（すりおろし）", "ねぎ（小口切り）適量", "大葉 3〜4枚"],
      steps: [
        "三枚おろしにしたアジから皮を引き、粗みじんに切る。",
        "まな板の上にアジ・味噌・生姜・ねぎ・大葉を乗せ、包丁でたたきながら混ぜ合わせる。",
        "好みの粘り気になったら器に盛り完成。ご飯や冷酒によく合う。",
      ],
    },
    {
      slug: "shioyaki",
      name: "アジの塩焼き",
      places: ["家のキッチン", "キャンプ場", "釣り場その場"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩"],
      servings: "2人分", time: "20分",
      ingredients: ["アジ 2尾", "塩 適量", "大根おろし（あれば）", "レモン（あれば）"],
      steps: [
        "アジのうろこ・エラ・内臓を取り除き、洗って水気を拭く。",
        "両面に塩をふり、15分置いて余分な水分を出す。",
        "グリルまたは網を中火で予熱し、アジを並べる。",
        "片面5分ずつ、こんがりと焼き色がつくまで焼いて完成。",
      ],
    },
    {
      slug: "munieru",
      name: "アジのムニエル",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["塩", "バター", "薄力粉"],
      servings: "2人分", time: "20分",
      ingredients: ["アジ 2尾（三枚おろし）", "塩こしょう 少々", "薄力粉 大さじ2", "バター 20g", "レモン（あれば）"],
      steps: [
        "三枚おろしのアジに塩こしょうをして5分置き、薄力粉を薄くまぶす。",
        "フライパンにバターを熱し、中火でアジの皮面から焼く。",
        "2〜3分で焼き色がついたら裏返し、さらに2分焼く。",
        "仕上げにバターをひとかけ足し、全体にからめてレモンを絞って完成。",
      ],
    },
    {
      slug: "nanbanzuke",
      name: "アジの南蛮漬け",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["酢", "醤油", "砂糖", "片栗粉", "サラダ油"],
      servings: "2人分", time: "30分",
      ingredients: ["アジ 2尾（三枚おろし）", "酢 大さじ3", "醤油 大さじ2", "砂糖 大さじ1", "水 大さじ2", "片栗粉 大さじ2", "サラダ油 揚げ油", "玉ねぎ・ピーマン（あれば）"],
      steps: [
        "酢・醤油・砂糖・水を混ぜてタレを作る。",
        "アジに塩こしょうして片栗粉をまぶし、180℃の油で揚げる。",
        "揚げたてのアジを熱いうちにタレに漬ける。",
        "薄切りにした玉ねぎ・ピーマンも一緒に漬けて、30分以上おいたら完成。",
      ],
    },
    {
      slug: "butter-shoyu-yaki",
      name: "アジのバター醤油焼き",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["バター", "醤油"],
      servings: "2人分", time: "15分",
      ingredients: ["アジ 2尾（三枚おろし）", "バター 15g", "醤油 大さじ1", "塩こしょう 少々"],
      steps: [
        "アジに塩こしょうをして下味をつける。",
        "フライパンにバターを溶かし、中火でアジの皮面から焼く。",
        "焼き色がついたら裏返し、醤油を回しかけて全体にからめる。",
        "バターが香ばしく焦げたら完成。ご飯がすすむ一品。",
      ],
    },
    {
      slug: "tataki",
      name: "アジのたたき",
      places: ["家のキッチン"],
      tools: ["包丁まな板あり", "なんでもある"],
      seasonings: ["醤油", "生姜"],
      servings: "2人分", time: "15分",
      ingredients: ["アジ 2尾（三枚おろし）", "醤油 大さじ1.5", "生姜（すりおろし）小さじ1", "ねぎ（小口切り）適量", "みょうが（あれば）"],
      steps: [
        "三枚おろしのアジを皮目を上にして網の上に置き、バーナーまたは直火で皮面をさっと炙る。",
        "すぐに氷水に取り、水気をしっかり拭く。",
        "食べやすい大きさに切り、器に盛る。",
        "生姜・ねぎをのせ、醤油をかけて完成。",
      ],
    },
  ],

  "サバ": [
    {
      slug: "misoni",
      name: "サバの味噌煮",
      places: ["家のキッチン"],
      tools: ["なんでもある", "フライパン"],
      seasonings: ["味噌", "みりん", "砂糖", "料理酒", "生姜"],
      servings: "2人分", time: "25分",
      ingredients: ["サバ 半身2切れ", "味噌 大さじ2", "みりん 大さじ2", "砂糖 大さじ1", "料理酒 大さじ2", "水 150ml", "生姜 1かけ（薄切り）"],
      steps: [
        "サバの皮に切り込みを入れ、熱湯をかけて霜降りにし、水気を拭く。",
        "鍋に水・料理酒・みりん・砂糖・生姜を入れて煮立てる。",
        "サバを皮面を上にして入れ、落し蓋をして中火で10分煮る。",
        "味噌を溶き入れ、さらに5分煮て煮汁が半量になったら完成。",
      ],
    },
    {
      slug: "shioyaki",
      name: "サバの塩焼き",
      places: ["家のキッチン", "キャンプ場", "釣り場その場"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩"],
      servings: "2人分", time: "20分",
      ingredients: ["サバ 2切れ", "塩 適量", "大根おろし（あれば）", "すだち（あれば）"],
      steps: [
        "サバに塩をふり、15分置いて臭みを除く。出てきた水分を拭き取る。",
        "グリル・網を中火で予熱する。",
        "皮面を下にして並べ、5〜6分焼く。",
        "裏返してさらに3〜4分焼き、こんがりしたら完成。",
      ],
    },
    {
      slug: "shime-saba",
      name: "しめサバ",
      places: ["家のキッチン"],
      tools: ["包丁まな板あり", "なんでもある"],
      seasonings: ["塩", "酢", "砂糖"],
      servings: "2人分", time: "60分（漬け込み含む）",
      ingredients: ["サバ 半身2枚", "塩 大さじ2", "酢 100ml", "砂糖 大さじ1"],
      steps: [
        "サバを三枚おろしにして腹骨を取り、塩を全体にまぶして30分置く。",
        "塩を洗い流し、水気を拭く。",
        "酢と砂糖を混ぜた漬け酢にサバを浸し、20〜30分漬ける。",
        "酢から取り出して皮を引き、食べやすく切ったら完成。",
      ],
    },
    {
      slug: "tatsutaage",
      name: "サバの竜田揚げ",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["醤油", "みりん", "生姜", "片栗粉", "サラダ油"],
      servings: "2人分", time: "30分",
      ingredients: ["サバ 2切れ", "醤油 大さじ2", "みりん 大さじ1", "生姜（すりおろし）小さじ1", "片栗粉 大さじ3", "サラダ油 揚げ油"],
      steps: [
        "サバを一口大に切り、醤油・みりん・生姜で20分漬け込む。",
        "漬け汁を拭き取り、片栗粉をしっかりまぶす。",
        "180℃の油で3〜4分、カリッとするまで揚げる。",
        "油を切って盛り付け、レモンを絞って完成。",
      ],
    },
    {
      slug: "misoyaki",
      name: "サバのみそ焼き",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["グリル・網", "フライパン", "なんでもある"],
      seasonings: ["味噌", "みりん", "料理酒"],
      servings: "2人分", time: "25分",
      ingredients: ["サバ 2切れ", "味噌 大さじ2", "みりん 大さじ1", "料理酒 大さじ1"],
      steps: [
        "味噌・みりん・料理酒を混ぜてみそダレを作る。",
        "サバの両面にみそダレを塗り、15分置く。",
        "グリルまたはフライパンで中火で焼く。焦げやすいので火加減に注意。",
        "両面にきれいな焼き色がついたら完成。",
      ],
    },
    {
      slug: "acqua-pazza",
      name: "サバのアクアパッツァ",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["塩", "サラダ油", "にんにく"],
      servings: "2人分", time: "25分",
      ingredients: ["サバ 2切れ", "塩こしょう 少々", "にんにく 1かけ", "ミニトマト 8個", "アサリ（あれば）100g", "白ワイン（または料理酒）50ml", "水 100ml", "オリーブオイル（またはサラダ油）大さじ1", "パセリ（あれば）"],
      steps: [
        "サバに塩こしょうをする。にんにくは薄切りにする。",
        "フライパンにオイルとにんにくを入れて弱火で香りを出す。",
        "サバを皮面から入れ、中火で2分焼いて裏返す。",
        "ミニトマト・アサリ・白ワイン・水を加えて蓋をし、中火で8分蒸し煮にして完成。",
      ],
    },
  ],

  "イワシ": [
    {
      slug: "kabayaki",
      name: "イワシの蒲焼き",
      places: ["家のキッチン"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["醤油", "みりん", "砂糖", "料理酒"],
      servings: "2人分", time: "20分",
      ingredients: ["イワシ 4尾（手開き）", "醤油 大さじ2", "みりん 大さじ2", "砂糖 大さじ1", "料理酒 大さじ1", "薄力粉 少々", "サラダ油 少々"],
      steps: [
        "イワシを手開きにして内臓・骨を取り、水気を拭く。",
        "薄力粉を薄くまぶす。",
        "フライパンに油を熱し、イワシを皮面から中火で焼く。",
        "両面に焼き色がついたら醤油・みりん・砂糖・料理酒を合わせたタレを加え、絡めながら煮詰めて完成。",
      ],
    },
    {
      slug: "shioyaki",
      name: "イワシの塩焼き",
      places: ["家のキッチン", "キャンプ場", "釣り場その場"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩"],
      servings: "2人分", time: "20分",
      ingredients: ["イワシ 4尾", "塩 適量", "大根おろし（あれば）"],
      steps: [
        "イワシの内臓を取り除き、よく洗って水気を拭く。",
        "全体に塩をふり、10分置く。",
        "グリル・網で中火で両面をこんがり焼く（片面4〜5分）。",
        "大根おろしを添えて完成。",
      ],
    },
    {
      slug: "tsumirejiru",
      name: "イワシのつみれ汁",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["味噌", "生姜", "料理酒"],
      servings: "2〜3人分", time: "30分",
      ingredients: ["イワシ 4尾", "味噌 大さじ2〜3", "生姜（すりおろし）小さじ1", "料理酒 大さじ1", "だし汁 600ml", "ねぎ 少々"],
      steps: [
        "イワシを手開きにして皮と骨を取り除き、包丁でよく叩く。",
        "叩いたイワシに生姜・料理酒・片栗粉少々を加えてこね、小さく丸める。",
        "だし汁を沸かし、つみれを入れて3〜4分煮る。",
        "火を止めて味噌を溶き入れ、ねぎを散らして完成。",
      ],
    },
    {
      slug: "umeni",
      name: "イワシの梅煮",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["醤油", "みりん", "砂糖", "料理酒", "酢"],
      servings: "2人分", time: "30分",
      ingredients: ["イワシ 4尾", "梅干し 2個", "醤油 大さじ2", "みりん 大さじ2", "砂糖 大さじ1", "料理酒 大さじ2", "水 100ml", "生姜 1かけ"],
      steps: [
        "イワシの内臓を取り除き、霜降りにして水気を拭く。",
        "鍋に水・料理酒・みりん・砂糖・醤油・梅干し・生姜を入れて煮立てる。",
        "イワシを入れ、落し蓋をして中火で15分煮る。",
        "煮汁が少なくなり、イワシに照りが出たら完成。",
      ],
    },
    {
      slug: "fry",
      name: "イワシフライ",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["塩", "薄力粉", "サラダ油"],
      servings: "2人分", time: "25分",
      ingredients: ["イワシ 4尾（手開き）", "塩こしょう 少々", "薄力粉 大さじ3", "溶き卵 1個", "パン粉 適量", "サラダ油 揚げ油"],
      steps: [
        "イワシを手開きにして骨を取り、塩こしょうをする。",
        "薄力粉→溶き卵→パン粉の順に衣をつける。",
        "180℃の油で3分ほど揚げ、きつね色になったら完成。",
        "ウスターソースやタルタルソースで食べると絶品。",
      ],
    },
  ],

  "タイ": [
    {
      slug: "sashimi",
      name: "タイの刺身",
      places: ["家のキッチン"],
      tools: ["包丁まな板あり", "なんでもある"],
      seasonings: ["醤油", "わさび"],
      servings: "2人分", time: "20分",
      ingredients: ["タイ 半身", "醤油 適量", "わさび 適量", "大根（つま用）", "大葉"],
      steps: [
        "タイを三枚おろしにして皮を引く。",
        "血合い骨をピンセットで抜く。",
        "繊維を断つよう斜めに包丁を入れ、5〜7mm幅に切る。",
        "器に大根のつまと大葉を敷き、並べてわさびと醤油を添えて完成。",
      ],
    },
    {
      slug: "shioyaki",
      name: "タイの塩焼き",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩"],
      servings: "2人分", time: "25分",
      ingredients: ["タイ 1尾（または切り身2切れ）", "塩 適量", "すだち（あれば）"],
      steps: [
        "タイのうろこ・内臓を取り、水洗いして水気を拭く。",
        "全体にたっぷり塩をふり、20分置く。",
        "グリルで中火、両面を各5〜7分こんがり焼く。",
        "すだちを添えて完成。ひれが焦げそうなら塩でコーティングしておく。",
      ],
    },
    {
      slug: "tai-meshi",
      name: "タイめし",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["醤油", "みりん", "料理酒", "塩"],
      servings: "3〜4人分", time: "40分",
      ingredients: ["タイ 切り身2切れ", "米 2合", "醤油 大さじ2", "みりん 大さじ1", "料理酒 大さじ1", "塩 少々", "昆布 5cm", "ごま・三つ葉（あれば）"],
      steps: [
        "米を洗い、炊飯器に入れる。醤油・みりん・料理酒・塩を加え、2合の目盛りまで水を足す。",
        "タイを塩で下味をつけ、昆布と一緒に米の上に置く。",
        "通常モードで炊く。",
        "炊き上がったらタイの骨を取り除き、ほぐして混ぜ、ごまと三つ葉を散らして完成。",
      ],
    },
    {
      slug: "acqua-pazza",
      name: "タイのアクアパッツァ",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["塩", "サラダ油", "にんにく"],
      servings: "2人分", time: "25分",
      ingredients: ["タイ 切り身2切れ", "塩こしょう 少々", "にんにく 1かけ", "ミニトマト 8個", "アサリ（あれば）150g", "白ワイン（または料理酒）50ml", "水 100ml", "オリーブオイル（またはサラダ油）大さじ1"],
      steps: [
        "タイに塩こしょうをする。にんにくは薄切り。",
        "フライパンにオイルとにんにくを入れて弱火で香りを出す。",
        "タイを皮面から中火で2〜3分焼き、裏返す。",
        "ミニトマト・アサリ・白ワイン・水を加えて蓋をし、8分蒸し煮にして完成。",
      ],
    },
    {
      slug: "kabutoni",
      name: "タイの兜煮",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["醤油", "みりん", "砂糖", "料理酒"],
      servings: "2人分", time: "30分",
      ingredients: ["タイの頭（兜）1個を半割り", "醤油 大さじ3", "みりん 大さじ3", "砂糖 大さじ1.5", "料理酒 50ml", "水 100ml", "生姜 1かけ"],
      steps: [
        "タイの兜に熱湯をかけて霜降りにし、水洗いして水気を拭く。",
        "鍋に水・料理酒・みりん・砂糖・醤油・生姜を入れて煮立てる。",
        "タイを入れて落し蓋をし、中火で15〜20分煮る。",
        "煮汁が半量になり照りが出たら完成。",
      ],
    },
    {
      slug: "carpaccio",
      name: "タイのカルパッチョ",
      places: ["家のキッチン"],
      tools: ["包丁まな板あり", "なんでもある"],
      seasonings: ["塩", "サラダ油", "酢"],
      servings: "2人分", time: "15分",
      ingredients: ["タイ 半身（刺身用）", "塩こしょう 少々", "オリーブオイル（またはサラダ油）大さじ2", "レモン汁（または酢）大さじ1", "玉ねぎ（薄切り）少々", "ハーブ（あれば）"],
      steps: [
        "タイを薄く（3〜4mm）削ぎ切りにして皿に放射状に並べる。",
        "オイル・レモン汁・塩こしょうを混ぜてドレッシングを作る。",
        "タイの上に薄切り玉ねぎをのせ、ドレッシングをかける。",
        "冷蔵庫で10分冷やして完成。",
      ],
    },
  ],

  "スズキ": [
    {
      slug: "munieru",
      name: "スズキのムニエル",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["塩", "バター", "薄力粉"],
      servings: "2人分", time: "20分",
      ingredients: ["スズキ 切り身2切れ", "塩こしょう 少々", "薄力粉 大さじ2", "バター 20g", "レモン（あれば）", "パセリ（あれば）"],
      steps: [
        "スズキに塩こしょうをして5分置き、水気を拭いて薄力粉をまぶす。",
        "フライパンにバターを溶かし、中火でスズキを皮面から焼く。",
        "3分で焼き色がついたら裏返し、2分焼く。",
        "仕上げにバターをひとかけ加えてソースを作り、スズキにかけて完成。",
      ],
    },
    {
      slug: "sashimi",
      name: "スズキの刺身",
      places: ["家のキッチン"],
      tools: ["包丁まな板あり", "なんでもある"],
      seasonings: ["醤油", "わさび"],
      servings: "2人分", time: "20分",
      ingredients: ["スズキ 半身", "醤油 適量", "わさび 適量", "大根（つま用）", "大葉"],
      steps: [
        "スズキを三枚おろしにして皮を引く。",
        "血合い骨をピンセットで抜く。",
        "斜めに包丁を入れて5〜7mm幅に切る。",
        "器に盛り、わさびと醤油を添えて完成。",
      ],
    },
    {
      slug: "saute",
      name: "スズキのソテー",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["塩", "バター", "サラダ油"],
      servings: "2人分", time: "20分",
      ingredients: ["スズキ 切り身2切れ", "塩こしょう 少々", "バター 15g", "サラダ油 大さじ1", "にんにく（あれば）1かけ"],
      steps: [
        "スズキに塩こしょうをして水気を拭く。",
        "フライパンにサラダ油を熱し、皮面から中火で3分焼く。",
        "裏返してバターとにんにくを加え、バターを溶かしながら2分焼く。",
        "スプーンでバターをすくいながらかけて香ばしく仕上げて完成。",
      ],
    },
    {
      slug: "shioyaki",
      name: "スズキの塩焼き",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩"],
      servings: "2人分", time: "20分",
      ingredients: ["スズキ 切り身2切れ", "塩 適量", "すだち（あれば）"],
      steps: [
        "スズキに塩をふり、15分置いて水気を拭く。",
        "グリルを中火で予熱し、皮面を下にして並べる。",
        "5〜6分焼いて裏返し、3〜4分焼いて完成。",
      ],
    },
    {
      slug: "acqua-pazza",
      name: "スズキのアクアパッツァ",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["塩", "サラダ油", "にんにく"],
      servings: "2人分", time: "25分",
      ingredients: ["スズキ 切り身2切れ", "塩こしょう 少々", "にんにく 1かけ", "ミニトマト 8個", "オリーブ（あれば）", "白ワイン（または料理酒）50ml", "水 100ml", "サラダ油 大さじ1"],
      steps: [
        "スズキに塩こしょう。にんにくは薄切り。",
        "フライパンにオイルとにんにくを弱火で炒めて香りを出す。",
        "スズキを皮面から中火で2〜3分焼き、裏返す。",
        "トマト・白ワイン・水を加えて蓋をし、8分蒸し煮して完成。",
      ],
    },
    {
      slug: "carpaccio",
      name: "スズキのカルパッチョ",
      places: ["家のキッチン"],
      tools: ["包丁まな板あり", "なんでもある"],
      seasonings: ["塩", "サラダ油", "酢"],
      servings: "2人分", time: "15分",
      ingredients: ["スズキ 刺身用半身", "塩こしょう 少々", "オリーブオイル（またはサラダ油）大さじ2", "レモン汁（または酢）大さじ1", "玉ねぎ 少々"],
      steps: [
        "スズキを薄く削ぎ切りにして皿に並べる。",
        "オイル・レモン汁・塩こしょうを混ぜてドレッシングを作る。",
        "薄切り玉ねぎをのせ、ドレッシングをかけて10分冷やして完成。",
      ],
    },
  ],

  "ブリ": [
    {
      slug: "teriyaki",
      name: "ブリの照り焼き",
      places: ["家のキッチン"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["醤油", "みりん", "砂糖", "料理酒"],
      servings: "2人分", time: "20分",
      ingredients: ["ブリ 切り身2切れ", "醤油 大さじ2", "みりん 大さじ2", "砂糖 大さじ1", "料理酒 大さじ1", "サラダ油 少々"],
      steps: [
        "ブリに塩をふって10分置き、水気を拭く。",
        "フライパンに油を熱し、中火でブリを両面焼いて取り出す。",
        "同じフライパンに醤油・みりん・砂糖・料理酒を入れて煮立てる。",
        "ブリを戻してタレにからめながら照りが出るまで焼いて完成。",
      ],
    },
    {
      slug: "buri-daikon",
      name: "ブリ大根",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["醤油", "みりん", "砂糖", "料理酒", "生姜"],
      servings: "2〜3人分", time: "45分",
      ingredients: ["ブリ 切り身2切れ", "大根 1/2本", "醤油 大さじ3", "みりん 大さじ2", "砂糖 大さじ1", "料理酒 50ml", "水 200ml", "生姜 1かけ"],
      steps: [
        "大根を2cm厚さの半月切りにして下茹でする。ブリは霜降りにして水気を拭く。",
        "鍋に水・料理酒・みりん・砂糖・醤油・生姜を煮立てる。",
        "大根を入れて中火で10分煮る。",
        "ブリを加えて落し蓋をし、さらに15分煮て完成。",
      ],
    },
    {
      slug: "sashimi",
      name: "ブリの刺身",
      places: ["家のキッチン"],
      tools: ["包丁まな板あり", "なんでもある"],
      seasonings: ["醤油"],
      servings: "2人分", time: "15分",
      ingredients: ["ブリ 刺身用半身", "醤油 適量", "わさび（あれば）", "大葉"],
      steps: [
        "ブリを三枚おろしにして皮を引く。",
        "血合い骨を抜き、繊維を断つよう斜めに8mm厚に切る。",
        "器に大葉を敷いて並べ、わさびと醤油を添えて完成。",
      ],
    },
    {
      slug: "buri-shabu",
      name: "ブリしゃぶ",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["ポン酢", "ごま油"],
      servings: "2人分", time: "20分",
      ingredients: ["ブリ 刺身用半身（薄切り）", "昆布 10cm", "ポン酢 適量", "ごま油 少々", "ねぎ・もみじおろし（あれば）", "水菜・豆腐"],
      steps: [
        "昆布を水に30分浸してだし汁を作り、鍋で温める。",
        "ブリを刺身より薄めに切る。",
        "沸騰直前のだし汁にブリをさっとくぐらせ、色が変わったら取り出す。",
        "ポン酢とごま油を混ぜたタレで食べて完成。",
      ],
    },
    {
      slug: "kamayaki",
      name: "ブリのカマ焼き",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩", "醤油"],
      servings: "2人分", time: "25分",
      ingredients: ["ブリのカマ 2個", "塩 適量", "醤油 少々（仕上げ用）", "大根おろし（あれば）"],
      steps: [
        "ブリのカマに塩をふり、20分置いて水気を拭く。",
        "グリルを強火で予熱し、カマを並べる。",
        "焦げないよう中火で両面を各5〜8分こんがり焼く。",
        "仕上げに醤油を少し垂らして香ばしさを出して完成。大根おろしを添える。",
      ],
    },
    {
      slug: "arajiru",
      name: "ブリのアラ汁",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["味噌", "料理酒"],
      servings: "2〜3人分", time: "30分",
      ingredients: ["ブリのアラ 300g", "味噌 大さじ2〜3", "料理酒 大さじ2", "生姜 1かけ", "水 600ml", "ねぎ 少々"],
      steps: [
        "ブリのアラに塩をふって10分置き、熱湯で霜降りにして水洗いする。",
        "鍋に水・料理酒・生姜を入れ、アラを入れて中火で10分煮る。",
        "アクをこまめに取る。",
        "火を止めて味噌を溶き入れ、ねぎを散らして完成。",
      ],
    },
  ],

  "カサゴ": [
    {
      slug: "karaage",
      name: "カサゴの唐揚げ",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["醤油", "生姜", "片栗粉", "サラダ油"],
      servings: "2人分", time: "25分",
      ingredients: ["カサゴ 2尾", "醤油 大さじ2", "生姜（すりおろし）小さじ1", "料理酒 大さじ1", "片栗粉 大さじ3", "サラダ油 揚げ油"],
      steps: [
        "カサゴのうろこ・内臓を取り除き、数か所切り込みを入れる。",
        "醤油・生姜・料理酒で20分漬け込む。",
        "水気を拭いて片栗粉をしっかりまぶす。",
        "170℃の油で5〜6分、カリッとするまで揚げて完成。",
      ],
    },
    {
      slug: "nitsuke",
      name: "カサゴの煮付け",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["醤油", "みりん", "砂糖", "料理酒"],
      servings: "2人分", time: "25分",
      ingredients: ["カサゴ 2尾", "醤油 大さじ2.5", "みりん 大さじ2", "砂糖 大さじ1", "料理酒 50ml", "水 100ml", "生姜 1かけ"],
      steps: [
        "カサゴのうろこ・内臓を取り、切り込みを入れて霜降りにする。",
        "鍋に水・料理酒・みりん・砂糖・醤油・生姜を入れて煮立てる。",
        "カサゴを入れて落し蓋をし、中火で10〜12分煮る。",
        "煮汁が半量になり照りが出たら完成。",
      ],
    },
    {
      slug: "shioyaki",
      name: "カサゴの塩焼き",
      places: ["家のキッチン", "キャンプ場", "釣り場その場"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩"],
      servings: "2人分", time: "25分",
      ingredients: ["カサゴ 2尾", "塩 適量", "すだち（あれば）"],
      steps: [
        "カサゴのうろこと内臓を取り、水洗いして水気を拭く。",
        "全体に塩をふり、20分置く。",
        "グリル・網で中火、両面こんがり焼く（片面6〜8分）。",
        "すだちを添えて完成。",
      ],
    },
    {
      slug: "misoshiru",
      name: "カサゴの味噌汁",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["なんでもある", "フライパン"],
      seasonings: ["味噌"],
      servings: "2〜3人分", time: "20分",
      ingredients: ["カサゴ 1〜2尾（ぶつ切り）", "味噌 大さじ2", "だし汁（または水＋昆布）600ml", "豆腐（あれば）", "ねぎ 少々"],
      steps: [
        "カサゴを下処理して食べやすくぶつ切りにし、霜降りにして水洗いする。",
        "だし汁を沸かし、カサゴを入れて中火で10分煮る。",
        "アクを取りながら豆腐を加え、さらに2〜3分煮る。",
        "火を止めて味噌を溶き入れ、ねぎを散らして完成。",
      ],
    },
    {
      slug: "acqua-pazza",
      name: "カサゴのアクアパッツァ",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["塩", "サラダ油", "にんにく"],
      servings: "2人分", time: "25分",
      ingredients: ["カサゴ 1〜2尾", "塩こしょう 少々", "にんにく 1かけ", "ミニトマト 8個", "アサリ（あれば）", "白ワイン（または料理酒）50ml", "水 100ml", "サラダ油 大さじ1"],
      steps: [
        "カサゴのうろこ・内臓を取り、塩こしょうをする。",
        "フライパンにオイルとにんにくを弱火で炒めて香りを出す。",
        "カサゴを入れて両面に焼き色をつける。",
        "トマト・アサリ・白ワイン・水を加えて蓋をし、10分蒸し煮して完成。",
      ],
    },
  ],

  "メバル": [
    {
      slug: "nitsuke",
      name: "メバルの煮付け",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["醤油", "みりん", "砂糖", "料理酒"],
      servings: "2人分", time: "25分",
      ingredients: ["メバル 2尾", "醤油 大さじ2.5", "みりん 大さじ2", "砂糖 大さじ1", "料理酒 50ml", "水 100ml", "生姜 1かけ"],
      steps: [
        "メバルのうろこ・内臓を取り、切り込みを入れて霜降りにする。",
        "鍋に水・料理酒・みりん・砂糖・醤油・生姜を入れて煮立てる。",
        "メバルを入れ、落し蓋をして中火で10分煮る。",
        "煮汁が少なくなり照りが出たら完成。",
      ],
    },
    {
      slug: "shioyaki",
      name: "メバルの塩焼き",
      places: ["家のキッチン", "キャンプ場", "釣り場その場"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩"],
      servings: "2人分", time: "20分",
      ingredients: ["メバル 2尾", "塩 適量"],
      steps: [
        "メバルのうろこと内臓を取り、水洗いして水気を拭く。",
        "全体に塩をふり、15分置く。",
        "グリル・網で中火で両面こんがり焼く（片面5〜6分）。",
        "すだちや大根おろしを添えて完成。",
      ],
    },
    {
      slug: "karaage",
      name: "メバルの唐揚げ",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["醤油", "生姜", "片栗粉", "サラダ油"],
      servings: "2人分", time: "25分",
      ingredients: ["メバル 2尾", "醤油 大さじ2", "生姜（すりおろし）小さじ1", "料理酒 大さじ1", "片栗粉 大さじ3", "サラダ油 揚げ油"],
      steps: [
        "メバルの下処理をして数か所切り込みを入れ、醤油・生姜・料理酒で20分漬ける。",
        "水気を拭いて片栗粉をまぶす。",
        "170℃の油で5〜6分揚げてカリッとさせて完成。",
      ],
    },
    {
      slug: "sashimi",
      name: "メバルの刺身",
      places: ["家のキッチン"],
      tools: ["包丁まな板あり", "なんでもある"],
      seasonings: ["醤油"],
      servings: "2人分", time: "20分",
      ingredients: ["メバル 2尾", "醤油 適量", "わさび（あれば）", "大葉"],
      steps: [
        "メバルを三枚おろしにして皮を引く。",
        "骨を丁寧に取り除き、食べやすい厚さに切る。",
        "大葉を敷いた器に並べ、わさびと醤油を添えて完成。",
      ],
    },
    {
      slug: "acqua-pazza",
      name: "メバルのアクアパッツァ",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["塩", "サラダ油", "にんにく"],
      servings: "2人分", time: "25分",
      ingredients: ["メバル 2尾", "塩こしょう 少々", "にんにく 1かけ", "ミニトマト 6〜8個", "白ワイン（または料理酒）50ml", "水 100ml", "サラダ油 大さじ1"],
      steps: [
        "メバルのうろこ・内臓を取り、塩こしょう。にんにくは薄切り。",
        "フライパンにオイルとにんにくを弱火で炒め香りを出す。",
        "メバルを入れて両面に焼き色をつける。",
        "トマト・白ワイン・水を加えて蓋をし、10分蒸し煮して完成。",
      ],
    },
  ],

  "ハマチ": [
    {
      slug: "sashimi",
      name: "ハマチの刺身",
      places: ["家のキッチン"],
      tools: ["包丁まな板あり", "なんでもある"],
      seasonings: ["醤油"],
      servings: "2人分", time: "15分",
      ingredients: ["ハマチ 刺身用半身", "醤油 適量", "わさび（あれば）", "大葉"],
      steps: [
        "ハマチを三枚おろしにして皮を引く。",
        "血合い骨をピンセットで抜き、8mm幅に切る。",
        "器に大葉を敷いて並べ、わさびと醤油を添えて完成。",
      ],
    },
    {
      slug: "teriyaki",
      name: "ハマチの照り焼き",
      places: ["家のキッチン"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["醤油", "みりん", "砂糖", "料理酒"],
      servings: "2人分", time: "20分",
      ingredients: ["ハマチ 切り身2切れ", "醤油 大さじ2", "みりん 大さじ2", "砂糖 大さじ1", "料理酒 大さじ1", "サラダ油 少々"],
      steps: [
        "ハマチに塩をふって10分置き、水気を拭く。",
        "フライパンに油を熱し、両面を中火で焼いて取り出す。",
        "醤油・みりん・砂糖・料理酒を加えて煮立てる。",
        "ハマチを戻してタレをからめ、照りが出たら完成。",
      ],
    },
    {
      slug: "carpaccio",
      name: "ハマチのカルパッチョ",
      places: ["家のキッチン"],
      tools: ["包丁まな板あり", "なんでもある"],
      seasonings: ["塩", "サラダ油", "酢"],
      servings: "2人分", time: "15分",
      ingredients: ["ハマチ 刺身用半身", "塩こしょう 少々", "オリーブオイル（またはサラダ油）大さじ2", "レモン汁（または酢）大さじ1", "玉ねぎ 少々"],
      steps: [
        "ハマチを薄く削ぎ切りにして皿に並べる。",
        "オイル・レモン汁・塩こしょうを混ぜてドレッシングを作る。",
        "薄切り玉ねぎをのせ、ドレッシングをかけて冷やして完成。",
      ],
    },
    {
      slug: "arajiru",
      name: "ハマチのアラ汁",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["味噌"],
      servings: "2〜3人分", time: "25分",
      ingredients: ["ハマチのアラ 300g", "味噌 大さじ2", "料理酒 大さじ2", "生姜 1かけ", "水 600ml", "ねぎ 少々"],
      steps: [
        "アラに塩をふって10分置き、霜降りにして水洗いする。",
        "鍋に水・料理酒・生姜を入れアラを中火で10分煮る。",
        "アクを取り除く。",
        "火を止めて味噌を溶き、ねぎを散らして完成。",
      ],
    },
    {
      slug: "shioyaki",
      name: "ハマチの塩焼き",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩"],
      servings: "2人分", time: "20分",
      ingredients: ["ハマチ 切り身2切れ", "塩 適量", "すだち（あれば）"],
      steps: [
        "ハマチに塩をふり、15分置いて水気を拭く。",
        "グリル・網で中火で両面こんがり焼く（片面5〜6分）。",
        "すだちを添えて完成。",
      ],
    },
  ],

  "キス": [
    {
      slug: "tempura",
      name: "キスの天ぷら",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["塩", "薄力粉", "サラダ油"],
      servings: "2人分", time: "25分",
      ingredients: ["キス 4尾（手開き）", "薄力粉 大さじ3", "冷水 50ml", "卵 1/2個", "サラダ油 揚げ油", "塩 少々（天塩用）"],
      steps: [
        "キスを手開きにして骨を取り除く。水気をよく拭く。",
        "卵と冷水を混ぜ、薄力粉を加えてさっくり混ぜる（少しダマがあってOK）。",
        "キスに薄力粉をまぶしてから衣にくぐらせ、180℃の油で2〜3分揚げる。",
        "油を切り、天塩またはだし塩で食べて完成。",
      ],
    },
    {
      slug: "shioyaki",
      name: "キスの塩焼き",
      places: ["家のキッチン", "キャンプ場", "釣り場その場"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩"],
      servings: "2人分", time: "20分",
      ingredients: ["キス 4尾", "塩 適量", "すだち（あれば）"],
      steps: [
        "キスのうろこ・内臓を取り除き、水洗いして水気を拭く。",
        "全体に塩をふり、10分置く。",
        "グリル・網で中火で両面焼く（片面3〜4分）。",
        "すだちを絞って完成。",
      ],
    },
    {
      slug: "fry",
      name: "キスのフライ",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["塩", "薄力粉", "サラダ油"],
      servings: "2人分", time: "25分",
      ingredients: ["キス 4尾（手開き）", "塩こしょう 少々", "薄力粉 大さじ3", "溶き卵 1個", "パン粉 適量", "サラダ油 揚げ油"],
      steps: [
        "キスを手開きにして骨を取り、塩こしょうをする。",
        "薄力粉→溶き卵→パン粉の順に衣をつける。",
        "180℃の油で2〜3分、きつね色になるまで揚げる。",
        "タルタルソースやウスターソースを添えて完成。",
      ],
    },
    {
      slug: "nanbanzuke",
      name: "キスの南蛮漬け",
      places: ["家のキッチン"],
      tools: ["なんでもある"],
      seasonings: ["酢", "醤油", "砂糖", "片栗粉", "サラダ油"],
      servings: "2人分", time: "30分",
      ingredients: ["キス 4尾", "酢 大さじ3", "醤油 大さじ2", "砂糖 大さじ1", "水 大さじ2", "片栗粉 大さじ2", "サラダ油 揚げ油", "玉ねぎ・ピーマン（あれば）"],
      steps: [
        "酢・醤油・砂糖・水を合わせてタレを作る。",
        "キスに塩をふって片栗粉をまぶし、180℃で揚げる。",
        "揚げたてをすぐにタレに漬ける。",
        "薄切り野菜も一緒に漬けて30分以上おいて完成。",
      ],
    },
    {
      slug: "himonoyaki",
      name: "キスの干物焼き",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩"],
      servings: "2人分", time: "30分（干し時間含む）",
      ingredients: ["キス 4尾", "塩水（水100ml・塩大さじ1）"],
      steps: [
        "キスのうろこ・内臓を取り、手開きにする。",
        "塩水に10〜15分浸し、水気を拭いて風に当てる。",
        "表面が乾いたらグリル・網で中火で両面焼く（片面3〜4分）。",
        "皮がパリッとしたら完成。",
      ],
    },
  ],

  "ニジマス": [
    {
      slug: "shioyaki",
      name: "ニジマスの塩焼き",
      places: ["家のキッチン", "キャンプ場", "釣り場その場"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩"],
      servings: "2人分", time: "25分",
      ingredients: ["ニジマス 2尾", "塩 適量", "すだち（あれば）"],
      steps: [
        "ニジマスの内臓とエラを取り、腹の中をよく洗って水気を拭く。",
        "全体に塩をふり、10分置いて出てきた水分を拭き取る。",
        "尾びれと背びれに多めの塩をつける（焦げ防止）。",
        "グリル・網を予熱し、中火で片面7〜8分ずつ焼く。",
        "身に火が通り、皮に焼き色がついたら完成。",
      ],
    },
    {
      slug: "meuniere",
      name: "ニジマスのムニエル",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["塩", "薄力粉", "バター"],
      servings: "2人分", time: "20分",
      ingredients: ["ニジマス 2尾（三枚おろし）", "塩こしょう 少々", "薄力粉 大さじ2", "バター 20g", "レモン（あれば）"],
      steps: [
        "ニジマスを三枚におろし、塩こしょうをして10分置く。",
        "水気を拭き、薄力粉を薄くまぶす。余分な粉は落とす。",
        "フライパンにバターを溶かし、皮面から中火で3〜4分焼く。",
        "裏返して2〜3分焼き、火を通す。",
        "フライパンに残ったバターをかけ、レモンを絞って完成。",
      ],
    },
    {
      slug: "hoiruyaki",
      name: "ニジマスのホイル焼き",
      places: ["家のキッチン", "キャンプ場"],
      tools: ["グリル・網", "フライパン", "なんでもある"],
      seasonings: ["塩", "バター", "料理酒"],
      servings: "2人分", time: "30分",
      ingredients: ["ニジマス 2尾", "塩こしょう 少々", "玉ねぎ 1/2個（薄切り）", "しめじ 1/2パック", "バター 20g", "料理酒 大さじ2", "レモン（あれば）"],
      steps: [
        "ニジマスの内臓を取って洗い、水気を拭いて塩こしょうをする。",
        "アルミホイルに玉ねぎとしめじを敷き、その上にニジマスをのせる。",
        "バターをのせ、料理酒をふりかける。",
        "ホイルの口をしっかり閉じ、中火で15〜20分蒸し焼きにする。",
        "ホイルを開けて火の通りを確認し、レモンを絞って完成。",
      ],
    },
    {
      slug: "karaage",
      name: "ニジマスの唐揚げ",
      places: ["家のキッチン"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["醤油", "料理酒", "生姜", "片栗粉", "サラダ油"],
      servings: "2人分", time: "30分",
      ingredients: ["ニジマス 2尾（三枚おろし）", "醤油 大さじ2", "料理酒 大さじ1", "生姜（すりおろし）小さじ1", "片栗粉 大さじ4", "サラダ油 揚げ油"],
      steps: [
        "ニジマスを三枚におろし、一口大に切る。",
        "醤油・料理酒・生姜を合わせ、20分漬け込む。",
        "汁気を軽く切り、片栗粉をまぶす。",
        "170度の油で3〜4分揚げる。",
        "一度取り出し、180度に上げて1分揚げ直すとカリッとする。",
      ],
    },
    {
      slug: "nanbanzuke",
      name: "ニジマスの南蛮漬け",
      places: ["家のキッチン"],
      tools: ["フライパン", "なんでもある"],
      seasonings: ["酢", "醤油", "砂糖", "片栗粉", "サラダ油"],
      servings: "2人分", time: "40分（漬け込み含む）",
      ingredients: ["ニジマス 2尾（三枚おろし）", "片栗粉 大さじ3", "玉ねぎ 1/2個（薄切り）", "にんじん 1/3本（千切り）", "酢 100ml", "醤油 大さじ2", "砂糖 大さじ2", "水 50ml", "サラダ油 揚げ油"],
      steps: [
        "酢・醤油・砂糖・水を合わせて南蛮酢を作り、玉ねぎとにんじんを漬けておく。",
        "ニジマスを三枚におろして一口大に切り、片栗粉をまぶす。",
        "170度の油で3〜4分揚げる。",
        "揚げたてを熱いうちに南蛮酢に漬ける。",
        "30分以上漬けたら完成。冷やすとさらに味がなじむ。",
      ],
    },
    {
      slug: "kunsei",
      name: "ニジマスの燻製",
      places: ["キャンプ場", "家のキッチン"],
      tools: ["グリル・網", "なんでもある"],
      seasonings: ["塩", "砂糖"],
      servings: "2人分", time: "3時間（乾燥・燻煙含む）",
      ingredients: ["ニジマス 2尾", "塩 大さじ2", "砂糖 大さじ1", "水 500ml", "スモークチップ（さくらなど）ひとつかみ"],
      steps: [
        "ニジマスの内臓とエラを取り、腹の中をよく洗う。",
        "水・塩・砂糖を混ぜたソミュール液に1時間漬ける。",
        "取り出して流水で塩抜きし、水気を拭いて風通しのよい場所で1時間乾かす。表面が乾くまでが重要。",
        "スモークチップをセットし、80度前後で1〜1時間半燻す。",
        "身がほぐれるようになったら完成。冷ましてから食べると味が締まる。",
      ],
    },
  ],
};

const FISHING_LOGS = [
  {
    slug: "asaka-garden-nijimasu",
    title: "朝霞ガーデンで虹鱒釣り｜8時間で3匹、でも一番うれしかった一匹",
    date: "2026-07-20",
    location: "朝霞ガーデン（マス釣り堀）",
    species: "虹鱒",
    fish: "ニジマス",
    guides: ["kanri-tsuriba-mochikaeri", "reitou-hozon"],
    conditions: "晴れ・朝",
    method: "スプーン",
    catchCount: "3匹（8時間）",
    body: [
      "朝霞ガーデンは、虹鱒を放流したマス釣り堀。晴れた朝、道具箱に忍ばせたのはスプーンだけだった。",
      "投げて、巻いて、また投げる。単調に見える動作の繰り返しだが、いつどこで魚がスプーンに食いつくか分からない緊張感がある。この日は朝から粘り続けて、結局8時間で釣れたのは3匹。数だけ見ればあまり褒められた釣果ではないかもしれない。",
      "それでも一番うれしかったのは、最初に釣れた1匹だった。実はこのスプーン、生まれて初めて自分で選んで買ったルアーだった。売り場でどれにしようか迷いに迷って選んだ一本。それが実際に魚を連れてきてくれた瞬間は、数字以上の達成感があった。",
      "虹鱒は身が締まっていて、塩焼きやムニエルにするとくせがなく食べやすい魚。釣り堀で放流されている個体は下処理さえ丁寧にすれば十分美味しく食べられる。次はもう少し数を伸ばして、色々な調理法を試してみたい。",
    ],
  },
];

// 解説記事（/guide/）。釣行記と違い、釣りに行かなくても書けるストック型のコンテンツ。
//
// sections[].body の各要素は次のいずれか：
//   "文字列"                        … 段落
//   { list: ["…", "…"] }            … 箇条書き
//   { steps: ["…", "…"] }           … 番号つき手順
//   { alert: "…" }                  … 注意喚起の枠
//   { img: "/img/…jpg", caption: "…" } … 写真（未撮影のうちは要素ごと省く）
//
// 工程写真は撮影後にこの img 要素を追加するだけで反映される。
//
// gear（任意）… 記事末尾に出すAmazon検索リンク。{ name: 表示名, q: 検索語 }
//   本文で実際に触れた道具だけを並べる。列挙しすぎると記事の信頼を落とすので3〜5点まで。
const GUIDES = [
  {
    slug: "aji-shitagoshirae",
    title: "アジの下処理｜釣った日にやることと三枚おろしまで",
    date: "2026-07-29",
    fish: "アジ",
    description: "釣ったアジを美味しく食べるための下処理を、釣り場での締め方から三枚おろしまで順を追って解説。ゼイゴの外し方、アニサキス対策、保存方法まで。",
    lead: "アジは釣りやすく、食べても美味しい魚だ。ただし美味しさの大半は、料理の腕前ではなく、釣り場と台所での下処理で決まる。ここでは釣ったアジを持ち帰り、三枚おろしにするまでの手順をまとめる。",
    sections: [
      {
        h2: "釣り場でやること",
        body: [
          "アジの味を落とす一番の原因は、締めずに放置することだ。生きたまま氷の上に置いたり、バケツで泳がせたまま時間を過ごすと、身に血が回って生臭さの元になる。",
          "20cm前後までの小型のアジなら、氷締めで十分だ。クーラーボックスに海水と氷を入れた「潮氷」を作り、釣れたアジをそこに直接入れる。真水の氷水ではなく海水を使うのは、真水に浸けると身が水を吸って水っぽくなるためだ。",
          "25cmを超えるサイズや、刺身にするつもりの個体は、エラの付け根を刃物で切って血を抜いてから潮氷に入れると、さらに生臭さが減る。",
          { alert: "潮氷が用意できない場合は、氷と魚が直接触れないようビニール袋に入れてからクーラーボックスへ。魚が氷焼けして身が白くなるのを防げる。" },
          "潮氷の作り方や締め方の使い分けは、<a href=\"/guide/mochikaeri/\">釣った魚の持ち帰り方</a>に図解つきでまとめてある。",
        ],
      },
      {
        h2: "用意する道具",
        body: [
          "特別な道具は要らない。家庭にあるもので足りる。",
          {
            list: [
              "包丁（出刃があれば理想だが、よく研いだ三徳包丁でも可）",
              "まな板",
              "ウロコ取り（包丁の背でも代用できる）",
              "キッチンペーパー",
              "ボウルまたは流水を使える環境",
            ],
          },
          "アジは骨が柔らかいので、大きな出刃がなくても捌ける。むしろ切れ味のほうが重要で、切れない包丁は身を潰して食感を悪くする。",
        ],
      },
      {
        h2: "手順1：ウロコを取る",
        body: [
          "流水にあてながら、尾から頭に向かって、包丁の背やウロコ取りで撫でるように削ぐ。アジのウロコは細かく剥がれやすいので力は要らない。",
          "背びれの根元、腹側、頭のすぐ後ろは取り残しやすい。指で撫でてざらつきが残っていないか確かめる。",
        ],
      },
      {
        h2: "手順2：ゼイゴを取る",
        body: [
          "アジ特有の工程がこれだ。尾の付け根から体の中ほどにかけて、側線の上に硬いトゲ状のウロコが並んでいる。これをゼイゴ（稜鱗）と呼ぶ。",
          { svg: "aji-zeigo" },
          "硬くて口に残るので、必ず取り除く。尾側から包丁を寝かせて差し入れ、刃を小刻みに前後させながら頭のほうへ削ぎ進める。皮一枚を薄く削ぐイメージで、身まで削らないよう注意する。",
          "両面にあるので、裏返して同じ作業をする。",
        ],
      },
      {
        h2: "手順3：頭と内臓を落とす",
        body: [
          {
            steps: [
              "胸びれの後ろから、頭の方へ斜めに包丁を入れる。裏返して同じ角度で入れ、頭を落とす。",
              "腹を肛門から頭側へ切り開く。",
              "内臓を指か包丁の先でかき出す。",
              "背骨に沿って走る血合い（黒っぽい血の塊）を、包丁の先や指の腹で掻き出す。",
            ],
          },
          { svg: "aji-head-cut" },
          "血合いが残ると生臭さの原因になるので、ここは丁寧にやる価値がある。",
        ],
      },
      {
        h2: "手順4：洗って水気を拭く",
        body: [
          "腹の中を流水でよく洗う。歯ブラシを使うと血合いの残りが落としやすい。",
          "洗い終えたら、キッチンペーパーで水気を徹底的に拭き取る。表面も腹の中も乾かす。",
          { alert: "水気を残したままにすると、傷みが早くなり、焼いても揚げても仕上がりが水っぽくなる。ここを省略しないこと。" },
        ],
      },
      {
        h2: "手順5：三枚おろし",
        body: [
          "刺身やフライにするなら三枚におろす。塩焼きや煮付けなら、ここまでの状態で調理できる。",
          {
            steps: [
              "頭側を右、背を手前にして置き、背側から包丁を入れる。背骨に沿って尾まで浅く切り込みを入れる。",
              "同じ線をなぞりながら、少しずつ深く切り進める。背骨に当たる感触を頼りにする。",
              "尾の付け根まで切れたら、包丁を寝かせて身を背骨から離す。これで片身が取れる。",
              "裏返して同じ手順を繰り返し、二枚目の身を取る。",
              "腹骨を薄く削ぎ取る。",
              "血合い骨は骨抜きで抜くか、細く切り取る。",
            ],
          },
          { svg: "aji-sanmai" },
          "一度で切り離そうとせず、浅い切り込みを何度か繰り返すほうが失敗しない。",
        ],
      },
      {
        h2: "刺身にする場合の注意",
        body: [
          { alert: "アジにはアニサキスをはじめとする寄生虫が付いていることがある。生食にはリスクが伴う。" },
          "アニサキスは内臓に多く寄生し、魚が死んだあと時間の経過とともに筋肉へ移動する。釣ったその日に内臓を取り除くことが、リスクを下げる最も効果的な手段になる。",
          "身を捌いたら、明るい場所で身の表面と内部を目視で確認する。白く細い糸状の虫が見えたら取り除く。",
          "確実に死滅させるには、加熱（中心温度70度以上）か冷凍（マイナス20度で24時間以上）が必要だ。家庭用冷凍庫は温度が足りない場合があるため、生食を選ぶ以上、目視だけではリスクが残ることは理解しておきたい。",
          "アニサキスについては<a href=\"/guide/saba-shitagoshirae-anisakis/\">サバの下処理とアニサキス対策</a>の記事で詳しく扱っている。酢や塩では死なないことなど、生食の前に知っておきたい点をまとめてある。",
          "皮を引く場合は、頭側の皮を指でつまみ、尾に向かって一気に剥く。アジの皮は手で剥ける。",
        ],
      },
      {
        h2: "保存の目安",
        body: [
          {
            list: [
              "当日中に食べる：三枚おろしにして、キッチンペーパーで包み冷蔵庫へ",
              "翌日に食べる：ワタとエラを抜いた状態で、水気を拭いてラップに包み冷蔵",
              "数日以上おく：三枚おろしにして水気を拭き、1回分ずつラップで包んで冷凍",
            ],
          },
          "冷凍する場合は、空気に触れる面積を減らすほど劣化が遅い。ラップでぴったり包んだうえで保存袋に入れ、空気を抜く。",
          "干物にするのも保存としては有効だ。開いて塩水に漬けてから一晩干せばよい。手順は<a href=\"/guide/himono/\">干物の作り方</a>にまとめてある。",
        ],
      },
      {
        h2: "ここまでできたら",
        body: [
          "下処理さえきちんとできていれば、あとはどう料理しても美味しく食べられる。塩焼き、フライ、なめろう、南蛮漬け、刺身。アジは応用が利く魚だ。",
        ],
      },
    ],
  },
  {
    slug: "saba-shitagoshirae-anisakis",
    title: "サバの下処理とアニサキス対策｜生で食べるなら冷凍が必須",
    date: "2026-07-29",
    fish: "サバ",
    description: "釣ったサバの下処理を、釣り場での締め方から三枚おろしまで解説。アニサキスがどこにいるか、酢では死なない理由、冷凍と加熱の条件、ヒスタミン食中毒への注意までまとめています。",
    lead: "サバは釣れると嬉しい魚だが、扱いを間違えると食べられなくなる魚でもある。傷みが早く、生食にはアニサキスのリスクがつきまとう。ここでは釣ったサバを持ち帰って捌くまでの手順と、生で食べる場合に何が必要になるのかをまとめる。",
    sections: [
      {
        h2: "サバは「生き腐れ」と呼ばれるほど傷みが早い",
        body: [
          "「サバの生き腐れ」という言い回しがある。生きているうちから腐り始めているように思えるほど、死後の劣化が速いという意味だ。実際サバは筋肉に含まれる酵素の働きが強く、脂も多い。見た目がまだ綺麗でも、中身の傷みが進んでいることがある。",
          "だから他の魚以上に、釣ってから台所に着くまでの温度管理が効いてくる。ここを外すと、あとの工程をどれだけ丁寧にやっても味は戻らない。",
        ],
      },
      {
        h2: "釣り場でやること",
        body: [
          {
            steps: [
              "釣れたらすぐ、エラの付け根に刃物を入れて血を抜く。サバは血が多く、残ると生臭さが強く出る。",
              "クーラーボックスに海水と氷を入れた「潮氷」を作り、そこへ入れて一気に冷やす。真水の氷水は身が水を吸うので使わない。",
              "クーラーボックスは日陰に置き、開け閉めを減らす。",
            ],
          },
          { alert: "サバを常温に置くのは避ける。味が落ちるだけでなく、後述するヒスタミン食中毒の原因にもなる。こればかりは加熱しても取り返しがつかない。" },
          "潮氷の作り方や血抜きの位置は、<a href=\"/guide/mochikaeri/\">釣った魚の持ち帰り方</a>に図解つきでまとめてある。",
        ],
      },
      {
        h2: "アニサキスとは何か",
        body: [
          "アニサキスは、魚介類に寄生する線虫の幼虫だ。長さ2〜3cm、太さ0.5〜1mmほどの白い糸状で、渦を巻いた状態で潜んでいることが多い。サバのほか、アジ、イワシ、サンマ、カツオ、サケ、イカなど幅広い魚介に寄生する。",
          "生きたまま食べてしまうと、胃や腸の壁に潜り込もうとする。数時間から十数時間後に、みぞおちの激しい痛みと吐き気が出るのが典型的な胃アニサキス症だ。基本的な治療は内視鏡での摘出になる。",
          { svg: "saba-anisakis-zone" },
          "重要なのは寄生している場所だ。魚が生きている間、アニサキスは内臓（腹腔内）に集中している。ところが魚が死んで時間が経つと、内臓から腹側の筋肉へと移動してくる。",
          "つまり、釣ったその日のうちに内臓を取り除くことが、リスクを下げる最も現実的で効果の大きい手段になる。持ち帰ってから翌日まで丸のまま冷蔵、というのが一番まずい。",
        ],
      },
      {
        h2: "効くのは加熱と冷凍だけ",
        body: [
          "アニサキスを確実に死滅させる方法は、次の2つに限られる。",
          {
            list: [
              "加熱：中心温度70度以上（60度なら1分以上）",
              "冷凍：マイナス20度で24時間以上",
            ],
          },
          "一方で、よく効くと思われがちだが実際には効かないものがある。",
          {
            list: [
              "酢（しめても死なない）",
              "塩",
              "醤油",
              "わさび",
              "レモン汁",
            ],
          },
          { alert: "「しめサバにすれば安全」は誤りだ。酢でアニサキスは死なない。しめサバを作るなら、冷凍の工程を別に挟む必要がある。" },
          "「よく噛めば大丈夫」という話も聞くが、噛み切れれば確かに死ぬというだけで、確実に噛み切れる保証はない。予防法として当てにしないほうがいい。",
          "家庭で冷凍する場合には注意点がある。家庭用冷凍庫はマイナス18度前後の設定が多く、マイナス20度に届かないことがある。ドアの開閉でさらに温度は上がる。現実的な折衷案としては、開け閉めの影響が少ない奥のほうに置き、48時間以上と時間を長めに取ることになる。それでも業務用の急速冷凍ほど確実ではない、という前提は持っておきたい。",
        ],
      },
      {
        h2: "下処理の手順",
        body: [
          "サバはウロコが細かく、アジのゼイゴのような硬い部位もない。工程そのものはアジより単純だ。",
          {
            steps: [
              "流水にあてながら、包丁の背で軽くウロコを落とす。ほとんど気にならない量なので力は要らない。",
              "胸びれの後ろから頭のほうへ斜めに包丁を入れ、裏返して同じ角度で入れて頭を落とす。",
              "腹を肛門から頭側へ切り開き、内臓を取り出す。",
              "背骨に沿った血合いを、包丁の先や指の腹で掻き出す。",
              "腹の中を流水でよく洗う。歯ブラシがあると血合いの残りが落としやすい。",
              "キッチンペーパーで水気を徹底的に拭き取る。表面も腹の中も乾かす。",
            ],
          },
          { alert: "内臓を取り出すときに破らないこと。アニサキスが最も多くいる場所なので、中身を身にこすりつけると意味がなくなる。破ってしまったら、その部分を切り落とすか、生食を諦めて加熱に回す。" },
          "水気を残すと傷みが早まり、焼いても揚げても仕上がりが水っぽくなる。サバは特にここが響く。",
        ],
      },
      {
        h2: "三枚おろしと、どこを重点的に見るか",
        body: [
          "三枚おろしの手順自体はアジと同じで、背側から背骨に沿って浅い切り込みを何度かなぞりながら深くしていく。詳しい手順は<a href=\"/guide/aji-shitagoshirae/\">アジの下処理</a>の記事に図解つきでまとめてあるので、そちらを参照してほしい。",
          { svg: "saba-harami-check" },
          "おろしたら、腹身（腹側の薄い部分）を重点的に確認する。ここは内臓に接していた場所で、移動してきたアニサキスが最も見つかりやすい。血合い骨のまわりも見落としやすいので、指で撫でながら見ていく。",
        ],
      },
      {
        h2: "目視での確認のしかた",
        body: [
          "目視は万全ではないが、やる価値はある。見つけやすくするコツがいくつかある。",
          { svg: "saba-light-check" },
          {
            list: [
              "明るい場所で行う。身の下から明かりを当てると、白い糸が影として浮かびやすい",
              "刺身に引くときは薄めに切る。厚いと中に隠れたものが見えない",
              "切った身は一切れずつ両面を見る",
              "ブラックライト（紫外線ライト）を使うと、アニサキスが青白く光って見つけやすくなる",
            ],
          },
          "それでも身の奥に潜んでいるものは見えない。目視だけで生食する以上、リスクはゼロにならないことは理解しておきたい。",
        ],
      },
      {
        h2: "しめサバを作るなら",
        body: [
          "しめサバは、サバ料理のなかでも特にアニサキス事故が多い。酢で死なないことを知らずに作られてしまうためだ。",
          "作るなら、次のどこかに冷凍の工程を必ず挟む。",
          {
            steps: [
              "三枚におろして腹骨を取る。",
              "塩を全体にまぶして30分ほど置き、洗って水気を拭く。",
              "酢に漬けて締める。",
              "この前後どちらかのタイミングで、マイナス20度で24時間以上（家庭用冷凍庫なら48時間以上）冷凍する。",
            ],
          },
          "味付けの分量や漬け時間は<a href=\"/recipe/saba-shime-saba/\">しめサバのレシピ</a>にまとめてある。",
          { alert: "アニサキスアレルギーの人は、加熱や冷凍で虫が死んでいても、残った成分に反応して症状が出ることがある。過去にサバで蕁麻疹や強い症状が出たことがある場合は、自己判断せず医師に相談してほしい。" },
        ],
      },
      {
        h2: "もうひとつのリスク：ヒスタミン食中毒",
        body: [
          "サバで注意すべきは寄生虫だけではない。",
          "サバのような赤身魚には、ヒスチジンというアミノ酸が多く含まれている。これが常温に置かれると、細菌の働きでヒスタミンに変わる。ヒスタミンを大量に摂ると、食後30分から1時間ほどで顔の紅潮、蕁麻疹、頭痛といった症状が出る。",
          { alert: "ヒスタミンは加熱しても分解しない。一度できてしまうと、焼いても煮ても取り除けない。防ぐ方法は温度管理しかない。" },
          {
            list: [
              "釣ったらすぐ潮氷で冷やす",
              "買ってきたらすぐ冷蔵・冷凍する",
              "解凍は常温ではなく冷蔵庫で行う",
              "口に入れたときに舌がピリピリしたら、それ以上食べない",
            ],
          },
        ],
      },
      {
        h2: "保存の目安",
        body: [
          {
            list: [
              "当日中に食べる：三枚おろしにして、キッチンペーパーで包み冷蔵庫へ",
              "翌日に食べる：内臓とエラを抜いた状態で、水気を拭いてラップに包み冷蔵。生食には向かない",
              "数日以上おく：三枚おろしにして水気を拭き、1回分ずつラップで包んで冷凍",
            ],
          },
          "サバは冷凍しても味が落ちにくいほうの魚だ。空気に触れる面積を減らすほど劣化が遅いので、ラップでぴったり包んだうえで保存袋に入れ、空気を抜く。",
          "「明日刺身にしよう」と丸のまま冷蔵するのが最も避けたいパターンになる。生食のつもりなら、その日のうちに内臓を抜いて冷凍まで進めてしまうほうがいい。",
        ],
      },
      {
        h2: "まとめ",
        body: [
          "サバは扱いに気を使う魚だが、押さえるべき点は多くない。釣ったら即冷やす。その日のうちに内臓を抜く。生で食べるなら冷凍する。この3つでリスクの大半は下がる。",
          "加熱してしまえば寄生虫の心配はなくなるので、味噌煮や塩焼きから入るのも十分にいい選択だ。",
        ],
      },
    ],
  },
  {
    slug: "mochikaeri",
    category: "hozon",
    title: "釣った魚の持ち帰り方｜潮氷の作り方と締め方の使い分け",
    date: "2026-07-29",
    description: "釣った魚を美味しく持ち帰るための手順を解説。潮氷の作り方、サイズ別の締め方、血抜きの位置、氷焼けを防ぐコツ、帰り道の温度管理までまとめています。",
    gear: [
      { name: "クーラーボックス", q: "クーラーボックス 釣り 15L" },
      { name: "水汲みバケツ", q: "水汲みバケツ 釣り ロープ付き" },
      { name: "フィッシングナイフ", q: "フィッシングナイフ 血抜き" },
      { name: "ジップ袋", q: "ジッパー付き保存袋 大" },
    ],
    lead: "釣った魚が美味しいかどうかは、料理より前の段階でほぼ決まっている。同じ場所で同じ日に釣れた魚でも、持ち帰り方が違うだけで味は明確に変わる。ここでは釣り場から自宅の台所までの扱い方をまとめる。特別な道具はほとんど要らない。",
    sections: [
      {
        h2: "味の大半は持ち帰り方で決まる",
        body: [
          { alert: "この記事は海釣り（海水魚）を前提にしている。管理釣り場のニジマスなど淡水魚には潮氷が使えないので、<a href=\"/guide/kanri-tsuriba-mochikaeri/\">管理釣り場の魚の持ち帰り方</a>を参照してほしい。" },
          "魚は死んだ瞬間から劣化が始まる。劣化を進めるのは主に温度と、身に残った血だ。この2つを釣り場で押さえられるかどうかが分かれ目になる。",
          "逆にここを外すと、あとから取り返す方法はない。どれだけ丁寧に捌いても、どれだけ良い調味料を使っても、生臭さは消えない。「釣った魚はスーパーの魚より美味しい」と言われるのは、この工程を自分で管理できるからだ。管理しなければ、むしろスーパーの魚に負ける。",
        ],
      },
      {
        h2: "用意するもの",
        body: [
          {
            list: [
              "クーラーボックス",
              "氷（クーラー容量の3分の1程度）",
              "海水を汲むためのバケツ（ロープ付きの水汲みバケツ）",
              "ナイフかハサミ（血抜き用。小型のもので足りる）",
              "ビニール袋またはジップ袋",
              "タオル",
            ],
          },
          "クーラーボックスは断熱材で保冷力が変わる。安い発泡スチロールのものは軽くて手軽だが、夏の日帰りだと氷が持たないことがある。発泡ウレタンの中価格帯なら、朝から夕方までは問題なく持つ。真空断熱パネルの高価格帯は泊まりがけや遠征向けで、堤防釣りには過剰だ。",
          "サイズは、アジやサバなど中小型を狙うなら10〜15リットルで足りる。青物や大型を狙う可能性があるなら25リットル以上を選ぶ。",
        ],
      },
      {
        h2: "潮氷を作る",
        body: [
          "潮氷（しおごおり）とは、海水と氷を混ぜてシャーベット状にしたものだ。これが釣り場での基本になる。",
          { svg: "cooler-shioogori" },
          "真水の氷水ではなく海水を使う理由は2つある。",
          {
            list: [
              "真水に浸けると魚が水を吸い、身が水っぽくなって旨味も流れ出す",
              "海水は塩分のぶん凍る温度が低く（およそマイナス1.8度）、真水の氷水より低い温度まで冷やせる",
            ],
          },
          "作るのは釣りを始める前がいい。魚が釣れてから慌てて作ると、その間に魚が弱って温度も上がる。到着したらまず海水を汲み、氷を入れて冷やしておく。",
          { alert: "氷が溶けて薄まってきたら海水を足す。逆に海水ばかりで氷が少ないと、ただの生ぬるい水になってしまう。シャーベット状を保つのが目安。" },
        ],
      },
      {
        h2: "締め方は魚のサイズで決める",
        body: [
          "「締める」というと難しく聞こえるが、要は魚を早く確実に死なせて、身に血と熱を残さないようにすることだ。やり方はサイズで使い分ける。",
          {
            list: [
              "20cm前後まで（豆アジ、小サバ、イワシなど）：氷締め。潮氷にそのまま入れるだけでいい",
              "25cm以上、または刺身にするつもりの魚：血抜きをしてから潮氷へ",
              "大型の青物や高級魚：血抜きに加えて神経締め",
            ],
          },
          "小型の魚をわざわざ一匹ずつ血抜きする必要はない。数が多いうえ、血の量も少ないので、潮氷に放り込んで急冷するほうが結果的に鮮度が保てる。",
          "神経締めは、脊椎の中を通る神経にワイヤーを差し込んで破壊する方法だ。死後硬直を遅らせられるが、手間と道具が要る。堤防で釣れる中小型魚に対しては、労力に見合わないことが多い。まずは血抜きを確実にやるほうが効果が大きい。",
        ],
      },
      {
        h2: "血抜きのやり方",
        body: [
          { svg: "chinuki-era" },
          {
            steps: [
              "魚のエラ蓋を持ち上げる。",
              "エラの付け根（背骨側）に刃先を差し入れて切る。太い血管が通っているので、うまく切れれば血が出てくる。",
              "海水を張ったバケツに数分入れておく。心臓がまだ動いているうちのほうが血は抜けやすい。",
              "血が抜けたら、潮氷へ移す。",
            ],
          },
          { alert: "血の混じった水にそのまま入れっぱなしにしないこと。血抜き用のバケツと、冷やすための潮氷は分けるのが基本。" },
          "うまく切れているかは、水が赤く染まるかどうかで分かる。染まらない場合は切る位置が浅いか、場所がずれている。",
        ],
      },
      {
        h2: "冷やしすぎと氷焼けを防ぐ",
        body: [
          "冷やすことは重要だが、やり方を間違えると別の失敗が起きる。",
          { svg: "koori-yake" },
          "氷が身に直接触れ続けると、その部分だけが凍って白く変色する。これを氷焼けという。食べられなくはないが、食感が悪くなり見た目も落ちる。潮氷の中では水がクッションになるので起きにくいが、水を切って氷の上に直接置くと起きる。",
          "対策は単純で、ビニール袋やジップ袋に入れてから氷の上に置けばいい。袋越しでも冷気は十分に伝わる。",
        ],
      },
      {
        h2: "潮氷に入れっぱなしにしない",
        body: [
          "意外と知られていないのがこれだ。潮氷は急速に冷やすためのもので、何時間も浸けておくものではない。",
          "長時間浸けたままにすると、身が水を吸って水っぽくなる。目が白く濁ってくるのも浸けすぎのサインだ。",
          "目安として、しっかり冷えたら（30分から1時間程度）水から上げ、ビニール袋に入れてクーラーボックスの氷の上に戻す。これで冷たさは保ったまま、水を吸うのを止められる。",
          "釣りに集中していると忘れがちなので、クーラーを開けたついでに確認する癖をつけておくといい。",
        ],
      },
      {
        h2: "内臓は釣り場で抜くべきか",
        body: [
          "鮮度と寄生虫の観点だけを見れば、内臓は早く抜くほど良い。アニサキスは魚が死ぬと内臓から筋肉へ移動してくるので、釣り場で処理できるならそれに越したことはない。",
          { alert: "ただし、釣り場での魚の処理を禁止している場所は多い。海に内臓を捨てる行為も、場所によってはマナー違反とされる。事前にその釣り場のルールを確認すること。" },
          "現実的には、処理が禁止されている場所も多く、水場や作業スペースの確保も難しい。移動時間が1時間程度なら、しっかり冷やして持ち帰り、家に着いてすぐ処理するので十分間に合う。",
          "移動に何時間もかかるなら、処理が許されている場所で内臓だけでも抜いておく価値はある。",
        ],
      },
      {
        h2: "帰り道の管理",
        body: [
          {
            list: [
              "クーラーボックスは直射日光に当てない。夏の車のトランクは非常に高温になる",
              "可能なら車内の日陰に置く",
              "フタの開け閉めを減らす。開けるたびに冷気が逃げる",
              "寄り道はしない。買い物などで車内に長時間置くのは避ける",
            ],
          },
          "せっかく釣り場で丁寧に冷やしても、帰りの2時間で台無しになることがある。特に夏場は、ここが一番の落とし穴になる。",
        ],
      },
      {
        h2: "家に着いてから",
        body: [
          {
            steps: [
              "クーラーボックスから出し、まだ処理していなければ内臓とエラを抜く。",
              "腹の中を流水でよく洗う。",
              "キッチンペーパーで水気を徹底的に拭き取る。",
              "その日に食べる分と、冷凍する分に分ける。",
            ],
          },
          "魚種ごとの詳しい下処理は、<a href=\"/guide/aji-shitagoshirae/\">アジの下処理</a>と<a href=\"/guide/saba-shitagoshirae-anisakis/\">サバの下処理とアニサキス対策</a>にまとめてある。生で食べるつもりならサバの記事は先に読んでおいてほしい。",
        ],
      },
      {
        h2: "クーラーボックスの手入れ",
        body: [
          "使ったあとの手入れを省くと、次の釣行で魚に臭いが移る。",
          {
            list: [
              "中性洗剤で洗い、しっかりすすぐ",
              "完全に乾かす。水気が残るとカビと臭いの原因になる",
              "保管はフタを開けた状態で",
              "臭いが取れないときは、重曹を溶かした水を張って一晩置く",
            ],
          },
          "パッキンの溝や水抜き栓のまわりは汚れが残りやすいので、意識して洗う。",
        ],
      },
      {
        h2: "まとめ",
        body: [
          "持ち帰りで押さえるべき点は4つだけだ。釣る前に潮氷を作っておく。サイズに応じて締める。しっかり冷えたら水から上げて袋に入れる。帰り道も冷やし続ける。",
          "道具もほとんど増えない。バケツとナイフとビニール袋があれば始められる。これだけで、同じ魚が別物になる。",
        ],
      },
    ],
  },
  {
    slug: "iwashi-tebiraki",
    title: "イワシの手開き｜包丁を使わない下処理",
    date: "2026-07-29",
    fish: "イワシ",
    description: "イワシの手開きのやり方を図解つきで解説。包丁を使わず指だけで頭・内臓・背骨を外す手順と、コツ、下処理後の使い道までまとめています。",
    lead: "イワシは骨が柔らかく、包丁を使わずに指だけで開ける。この「手開き」を覚えておくと、まな板と包丁を出すのが面倒な日でも下処理ができる。数が釣れる魚なので、一匹あたりの手間が減る効果も大きい。慣れれば一匹30秒ほどだ。",
    sections: [
      {
        h2: "なぜ包丁を使わないのか",
        body: [
          "イワシは骨が細く柔らかいので、指で押すだけで背骨と身が分かれる。無理に包丁を入れるより、指のほうがきれいに外れることも多い。",
          "手開きの利点は他にもある。",
          {
            list: [
              "洗い物が減る（まな板と包丁を使わない）",
              "数が多いときに速い",
              "力加減が指に伝わるので、身を余計に切り落とさない",
              "包丁が使えない場所でも下処理できる",
            ],
          },
          "梅干し煮や蒲焼き、フライ、つみれなど、イワシ料理の多くは開いた状態から始まる。この工程を押さえておくと料理の幅が一気に広がる。",
        ],
      },
      {
        h2: "始める前に",
        body: [
          "手が冷たいほうが身が締まって扱いやすい。作業の前に手を冷水で冷やしておくといい。逆に、手が温かいまま長く触っていると身がゆるんで崩れやすくなる。",
          {
            list: [
              "ボウルに氷水を用意しておくと、途中で手を冷やせる",
              "受け皿かバットを手元に置く",
              "キッチンペーパーを多めに用意する",
            ],
          },
          { alert: "鮮度が落ちた個体は身がゆるく、手開きにすると崩れる。届いたばかり・釣ったばかりのものほどきれいに開ける。傷んでいるものは無理をせず包丁を使う。" },
        ],
      },
      {
        h2: "手順1：ウロコを落とす",
        body: [
          "イワシのウロコは非常に剥がれやすい。流水にあてながら、指の腹で尾から頭に向かって撫でるだけで落ちる。",
          "運搬の途中でほとんど落ちてしまっていることも多いので、ざらつきが残っている場所だけ確認すれば足りる。",
        ],
      },
      {
        h2: "手順2：頭とワタを取る",
        body: [
          { svg: "iwashi-atama" },
          {
            steps: [
              "胸びれの後ろに指をあて、頭を下に折る。骨が柔らかいので簡単に折れる。",
              "折った頭をそのまま下へ引く。うまくいくと内臓が頭にくっついて一緒に出てくる。",
              "残った内臓があれば、指でかき出す。",
              "腹の中を流水で洗い、血合いを落とす。",
            ],
          },
          "内臓が途中で切れてしまっても問題ない。腹を指で裂いて中を洗えばいい。イワシの腹は薄いので、指だけで簡単に開く。",
        ],
      },
      {
        h2: "手順3：親指を入れて開く",
        body: [
          { svg: "iwashi-hiraku" },
          "ここが手開きの核心になる。",
          {
            steps: [
              "腹側を手前にして持つ。",
              "背骨の上（腹の内側）に両方の親指を差し入れる。",
              "背骨に沿わせたまま、尾のほうへゆっくり滑らせる。",
              "親指が通った分だけ、身が左右に開いていく。",
            ],
          },
          { alert: "力を入れて押し広げると身が裂ける。骨の感触をなぞるように、力ではなく方向で動かすのがコツ。" },
          "途中で引っかかったら、いったん戻して角度を変える。無理に進めると身が割れる。",
        ],
      },
      {
        h2: "手順4：背骨を外す",
        body: [
          { svg: "iwashi-hone" },
          {
            steps: [
              "開いた状態で、背骨の頭側の端を指でつまむ。",
              "尾に向かって、ゆっくり引き剥がす。",
              "尾の付け根まで来たら、そこで背骨を折って取る。",
              "腹骨と背びれが残っていれば、指でつまんで引き抜く。",
            ],
          },
          "尾を残すかどうかは料理次第だ。フライや蒲焼きは尾を残したほうが見栄えがよく、つみれにするなら要らない。",
        ],
      },
      {
        h2: "仕上げ",
        body: [
          "開き終わったら、キッチンペーパーで水気をしっかり拭き取る。イワシは水っぽくなりやすいので、ここは省かないほうがいい。",
          "小骨が気になる場合は、指で撫でて残っているものを抜く。ただしイワシの小骨は加熱すれば気にならなくなるので、フライや揚げ物にするなら神経質になる必要はない。",
        ],
      },
      {
        h2: "うまくいかないときは",
        body: [
          {
            list: [
              "身が崩れる → 鮮度が落ちているか、手が温かい。氷水で手を冷やす",
              "背骨がきれいに外れない → 親指を滑らせる工程が浅い。もう一度背骨に沿わせ直す",
              "腹側が裂ける → 力が入りすぎ。押すのではなく滑らせる",
              "そもそも折れない → 胸びれより頭側で折ろうとしている。位置を後ろにずらす",
            ],
          },
          "最初の数匹は失敗して当然だ。イワシは数が釣れる魚なので、練習台には困らない。",
        ],
      },
      {
        h2: "開いたあとの使い道",
        body: [
          "手開きにしたイワシは、そのまま多くの料理に使える。",
          {
            list: [
              "フライ、天ぷら",
              "蒲焼き、生姜煮",
              "つみれ（包丁で叩くか、フードプロセッサーへ）",
              "干物（塩水に漬けて一晩干す）",
            ],
          },
          "手開きにした形はそのまま干物に使える。塩水の濃度と干し時間は<a href=\"/guide/himono/\">干物の作り方</a>にまとめてある。",
          "同じ手順は、アジやサンマなど骨の柔らかい魚にも応用できる。アジの場合は<a href=\"/guide/aji-shitagoshirae/\">アジの下処理</a>にあるとおりゼイゴを先に取る必要があるが、それ以外はほぼ同じだ。",
        ],
      },
      {
        h2: "まとめ",
        body: [
          "手開きは、覚えてしまえば包丁を出すより速い。頭を折る、親指を滑らせる、背骨を剥がす。この3つだけだ。",
          "うまくいかないときはたいてい「力を入れすぎ」か「手が温かい」のどちらかが原因になっている。",
        ],
      },
    ],
  },
  {
    slug: "reitou-hozon",
    category: "hozon",
    title: "釣った魚の冷凍と保存｜家庭の冷凍庫でどこまでできるか",
    date: "2026-07-29",
    description: "釣った魚の冷凍と保存を解説。下処理の順番、ラップと保存袋の使い分け、冷凍焼けを防ぐ方法、解凍のしかた、保存期間の目安までまとめています。",
    gear: [
      { name: "保存袋", q: "ジッパー付き保存袋 冷凍用 大" },
      { name: "業務用ラップ", q: "ラップ 業務用 30cm" },
      { name: "アルミトレー", q: "急速冷凍 アルミトレー" },
      { name: "真空パック器", q: "真空パック器 家庭用" },
    ],
    lead: "釣りは釣れるときにまとめて釣れる。その日に食べきれない分をどう保存するかで、翌週の食卓の質が決まる。冷凍は正しくやれば味をほとんど落とさないが、雑にやると一気にまずくなる。分かれ目は水気と空気の2つだ。",
    sections: [
      {
        h2: "冷凍する前に下処理を済ませる",
        body: [
          "丸のまま冷凍するのは、いちばん避けたいやり方だ。理由は3つある。",
          {
            list: [
              "内臓が残っていると、そこから傷みと臭みが広がる",
              "解凍してから捌くことになり、身がゆるんで扱いにくい",
              "かさばるので冷凍庫を圧迫する",
            ],
          },
          "内臓とエラを抜き、可能なら三枚おろしまで済ませてから冷凍する。これだけで解凍後の使い勝手がまったく違う。",
          "アニサキス対策としても、冷凍前の内臓処理には意味がある。詳しくは<a href=\"/guide/saba-shitagoshirae-anisakis/\">サバの下処理とアニサキス対策</a>にまとめてある。",
        ],
      },
      {
        h2: "水気を拭く",
        body: [
          "冷凍で失敗する原因のほとんどがこれだ。",
          "表面に残った水分は、凍るときに大きな氷の結晶になる。この結晶が細胞を壊し、解凍したときに旨味の含まれた水分（ドリップ）として流れ出す。パサついた食感になるのはこのためだ。",
          { alert: "洗ったあと、キッチンペーパーで表面と腹の中を徹底的に拭く。「もういいだろう」と思ったところからもう一往復するくらいでちょうどいい。" },
        ],
      },
      {
        h2: "包み方",
        body: [
          { svg: "reitou-tsutsumi" },
          {
            steps: [
              "水気を拭く。",
              "1回分ずつに小分けし、ラップでぴったり包む。身とラップの間に隙間を作らない。",
              "保存袋に入れ、空気を抜いて閉じる。",
              "日付と魚種を書いておく。",
            ],
          },
          "小分けにするのは、使う分だけ解凍するためだ。一度解凍したものを再冷凍すると味が大きく落ちる。",
          "日付を書くのは地味だが効く。書いていないものは、結局いつまでも冷凍庫に残る。",
        ],
      },
      {
        h2: "空気を抜く",
        body: [
          { svg: "reitou-kuuki" },
          "袋の中に空気が残っていると、そこにあった水分が霜になり、身の表面が乾いてスカスカになる。これが冷凍焼けだ。腐っているわけではないので食べられるが、味も食感も明確に落ちる。",
          "家庭でできる空気の抜き方は簡単だ。",
          {
            list: [
              "袋の口を少しだけ開けたまま、水を張ったボウルに沈める。水圧で空気が押し出されるので、そこで口を閉じる",
              "ストローで吸い出す",
              "真空パック器を使う（頻度が高いなら投資する価値がある）",
            ],
          },
          "水を使う方法は道具が要らないうえ効果が高い。まずはこれで十分だ。",
        ],
      },
      {
        h2: "早く凍らせる",
        body: [
          "ゆっくり凍るほど氷の結晶は大きくなり、細胞を壊す。逆に急速に凍らせるほど結晶は小さく、解凍後の劣化が少ない。",
          {
            list: [
              "金属トレー（アルミバット）の上に置く。熱が速く逃げる",
              "冷凍庫の急速冷凍機能があれば使う",
              "一度にたくさん入れない。庫内の温度が上がって全体が遅くなる",
              "食材どうしを重ねず、平らに並べる",
            ],
          },
          "アルミトレーは百円ショップのものでも効果がある。特別な道具は要らない。",
        ],
      },
      {
        h2: "解凍のしかた",
        body: [
          "凍らせ方より、解凍で失敗する人のほうが多い。",
          {
            list: [
              "冷蔵庫に移して半日〜一晩：最も味が落ちない。基本はこれ",
              "氷水に袋ごと沈める：30分〜1時間。急ぐときはこちら",
              "流水：早いが、袋が破れていると水が入って水っぽくなる",
              "常温：表面だけ先に傷むので避ける",
              "電子レンジ：加熱ムラができやすい。どうしてもの場合のみ",
            ],
          },
          { alert: "常温での解凍は、細菌が増えやすい温度帯に長く置くことになる。特に夏場は避ける。" },
          "煮付けや汁物にするなら、凍ったまま調理してしまう手もある。解凍中のドリップが出ないぶん、かえって味が落ちにくい。",
        ],
      },
      {
        h2: "保存期間の目安",
        body: [
          {
            list: [
              "冷蔵（下処理済み・当日〜翌日）：1〜2日。生食は当日中",
              "冷凍（白身魚）：3〜4週間",
              "冷凍（サバ・イワシなど脂の多い青魚）：2〜3週間。脂が酸化するぶん短い",
              "干物にして冷凍：1ヶ月程度",
            ],
          },
          "食べきれない量が釣れたときは、冷凍だけでなく<a href=\"/guide/himono/\">干物</a>にする手もある。味が変わるので、同じ魚が続いても飽きにくい。",
          "これは「安全に食べられる期間」ではなく「美味しく食べられる期間」の目安だ。冷凍庫の温度やドアの開閉頻度でも変わる。",
          "脂の多い魚ほど早めに食べたほうがいい。酸化した脂の臭いは、加熱しても消えない。",
        ],
      },
      {
        h2: "冷凍に向かないもの",
        body: [
          {
            list: [
              "刺身用として解凍後に生で食べる前提のもの（家庭用冷凍庫では品質を保ちにくい）",
              "一度解凍したものの再冷凍",
              "内臓を抜いていない魚",
            ],
          },
          "なお、アニサキス対策としての冷凍は別の話になる。この場合はマイナス20度で24時間以上という条件があり、家庭用冷凍庫では温度が届かないことがある。詳細は<a href=\"/guide/saba-shitagoshirae-anisakis/\">サバの記事</a>を参照してほしい。",
        ],
      },
      {
        h2: "まとめ",
        body: [
          "冷凍で押さえるのは4点だ。下処理を済ませる。水気を拭く。空気を抜く。早く凍らせる。",
          "そして解凍は冷蔵庫でゆっくり。これだけで、釣った日の魚が数週間後もきちんと美味しく食べられる。",
        ],
      },
    ],
  },
  {
    slug: "kanri-tsuriba-mochikaeri",
    category: "hozon",
    title: "管理釣り場の魚の持ち帰り方｜淡水では潮氷を使わない",
    date: "2026-08-01",
    fish: "ニジマス",
    description: "管理釣り場で釣ったニジマスの持ち帰り方を解説。淡水魚に潮氷を使わない理由、現地での締め方と内臓処理、保冷剤の使い方、淡水魚を生で食べてはいけない理由までまとめています。",
    gear: [
      { name: "小型クーラーボックス", q: "クーラーボックス 釣り 10L" },
      { name: "保冷剤", q: "保冷剤 釣り 長時間" },
      { name: "フィッシュナイフ", q: "フィッシングナイフ 小型" },
      { name: "ジッパー付き保存袋", q: "ジッパー付き保存袋 大 マチ付き" },
    ],
    lead: "管理釣り場は手ぶらに近い装備で行けるのが魅力だが、そのぶん持ち帰りの準備を忘れやすい。しかも海釣りの定番である潮氷は、淡水では使えない。ここでは管理釣り場で釣ったニジマスを、味を落とさず家まで運ぶ手順をまとめる。",
    sections: [
      {
        h2: "淡水魚に潮氷を使わない理由",
        body: [
          "海釣りでは、クーラーボックスに海水と氷を入れた「潮氷」に魚を沈めるのが基本になる。冷たい水は空気より圧倒的に速く熱を奪うので、魚を短時間で芯まで冷やせるからだ。",
          "ところが管理釣り場ではこれができない。海水が手に入らないうえ、真水の氷水に魚を浸けると身が水を吸ってしまうためだ。",
          { svg: "kanri-shinto" },
          "魚の身と真水では塩分濃度が違う。濃度の低いほうから高いほうへ水が移動するので、真水に浸けた魚の身は水を吸って膨らむ。焼いても揚げても水っぽく、身がぼやけた仕上がりになる。海水魚を海水の潮氷に入れるのが理にかなっているのは、濃度が近く、この移動が起きにくいからだ。",
          { alert: "「氷水に浸ければ冷える」は淡水では逆効果になる。淡水魚は<strong>濡らさずに冷やす</strong>のが原則。" },
          "海で釣った魚の持ち帰りについては、<a href=\"/guide/mochikaeri/\">釣った魚の持ち帰り方</a>にまとめてある。潮氷の作り方や氷焼けの防ぎ方は、そちらを参照してほしい。",
        ],
      },
      {
        h2: "行く前に確認しておくこと",
        body: [
          "管理釣り場には、その釣り場ごとのルールがある。受付やサイトで先に確認しておくと現地で困らない。",
          {
            list: [
              "持ち帰れる匹数の上限（放流量に応じて決まっていることが多い）",
              "リリース専用エリアかどうか（区画によって扱いが違う釣り場がある）",
              "内臓処理場・洗い場の有無（これがあるかどうかで持ち物が変わる）",
              "氷や保冷剤の販売があるか",
              "釣った魚の持ち帰り用の袋がもらえるか",
            ],
          },
          "内臓処理場がある釣り場なら、現地で腹を抜いて帰れる。これができると味の落ち方がまったく違うので、あるなら必ず使いたい。",
        ],
      },
      {
        h2: "持ち物",
        body: [
          {
            list: [
              "小型のクーラーボックス（10L前後で足りる）",
              "保冷剤または板氷（溶けて水が出にくいものが望ましい）",
              "ジッパー付きの保存袋、またはビニール袋",
              "キッチンペーパーか新聞紙",
              "小型のナイフ（現地で締める・腹を抜く場合）",
            ],
          },
          "海釣りのような大型クーラーは要らない。ニジマスは20〜30cm程度なので、数匹なら10Lで十分収まる。",
        ],
      },
      {
        h2: "釣ったらすぐ締める",
        body: [
          "生きたまま袋に入れて放置するのが、いちばん味を落とすやり方だ。魚は暴れているあいだに体内のエネルギーを使い切り、身に血と乳酸が回る。これが生臭さと身の柔らかさの原因になる。",
          "ニジマスのサイズなら、締め方は次の2つで足りる。",
          {
            steps: [
              "首折り：エラ蓋のすぐ後ろを持ち、頭を背側へ強く折り曲げる。背骨が折れて即死する。道具が要らないので管理釣り場ではこれが手軽。",
              "エラ切り：エラ蓋を持ち上げ、付け根に刃先を入れて切る。そのままバケツの水に数分入れておくと血が抜ける。",
            ],
          },
          "刺身にはしない魚なので、そこまで神経質になる必要はない。ただし「締めずに放置しない」の一点だけは守る価値がある。",
          "エラの切る位置は<a href=\"/guide/mochikaeri/\">釣った魚の持ち帰り方</a>に図解がある。海水魚の図だが、切る場所は淡水魚でも同じだ。",
        ],
      },
      {
        h2: "できれば現地で腹を抜く",
        body: [
          "内臓処理場のある釣り場なら、締めたあとに腹を抜いてしまう。手順は簡単だ。",
          {
            steps: [
              "肛門に刃先を入れ、頭のほうへ腹を切り開く。",
              "内臓を指でつまんで引き出す。ニジマスの内臓はまとまって取れる。",
              "背骨に沿った黒い血合いを、指の腹か爪でこすり落とす。",
              "水でさっと洗い、水気を切る。",
            ],
          },
          "内臓は消化酵素と細菌の供給源なので、残したまま持ち帰ると身の傷みが速い。現地で抜けるなら抜く。これが淡水・海水を問わず、味に効く一番大きな一手だ。",
          { alert: "洗ったあとは長く水に浸けない。ここでも身が水を吸う。さっと流してすぐ水気を拭く。" },
        ],
      },
      {
        h2: "袋に入れてから冷やす",
        body: [
          { svg: "kanri-hoshitsu" },
          "淡水魚を冷やすときの構成は「魚は乾かし、冷気だけを伝える」が基本になる。",
          {
            steps: [
              "魚の水気をキッチンペーパーか新聞紙で拭く。",
              "1匹ずつ、またはまとめてビニール袋・保存袋に入れる。",
              "クーラーボックスに保冷剤を敷き、その上に袋ごと置く。",
              "上からも保冷剤を当て、袋が直接氷や保冷剤に触れる面が偏らないようにする。",
            ],
          },
          "袋越しでも冷気は十分に伝わる。魚を直接氷に当てると、触れた部分だけが凍って白く変色する（氷焼け）ので、袋を一枚はさむ意味はここにもある。",
          "保冷剤が足りず板氷を使う場合は、溶けた水がクーラーの底に溜まる。そこに袋が沈むと結局水に浸かることになるので、水抜き栓を開けておくか、底に新聞紙を敷いておく。",
        ],
      },
      {
        h2: "帰宅後にやること",
        body: [
          "家に着いたらそのまま冷蔵庫に入れず、ひと手間かける。",
          {
            steps: [
              "現地で腹を抜いていなければ、ここで抜く。",
              "流水でぬめりと血合いを洗う。ニジマスは体表のぬめりが多く、これが臭みの元になる。指でこすりながら洗い流す。",
              "キッチンペーパーで表面と腹の中の水気を拭き取る。",
              "その日に食べるならラップで包んで冷蔵、後日ならラップ＋保存袋で冷凍。",
            ],
          },
          "冷凍のしかたは<a href=\"/guide/reitou-hozon/\">釣った魚の冷凍と保存</a>にまとめてある。水気を拭いてから包む、空気を抜く、早く凍らせる。この3点は淡水魚でも同じだ。",
        ],
      },
      {
        h2: "淡水魚は生で食べない",
        body: [
          { alert: "管理釣り場のニジマスを刺身で食べるのは避けたほうがいい。淡水魚には、海水魚とは別の寄生虫のリスクがある。" },
          "サケ・マス類には日本海裂頭条虫（サナダムシの一種）が、淡水魚全般には顎口虫や横川吸虫が寄生することがある。これらは海水魚のアニサキスと違い、人の体内で長く生存したり成虫になったりするものも含まれる。",
          "「養殖だから安全」と言われることもあるが、餌や飼育環境によってリスクの有無は変わる。市販の「サーモン」が生で流通しているのは、生食用の管理と冷凍処理を経ているからであって、釣り場の魚がそれと同じ状態にあるわけではない。",
          "したがって、ニジマスは加熱して食べるのが基本になる。塩焼き、ムニエル、ホイル焼き、唐揚げ。どれも身の淡白さが活きるので、加熱料理でも物足りなさはない。",
          "アニサキスなど海水魚側のリスクについては<a href=\"/guide/saba-shitagoshirae-anisakis/\">サバの下処理とアニサキス対策</a>にまとめてある。",
        ],
      },
      {
        h2: "まとめ",
        body: [
          "淡水の持ち帰りで押さえるのは3点だ。真水に浸けない。締めて、できれば現地で腹を抜く。袋に入れてから保冷剤で冷やす。",
          "海釣りの潮氷ほど強力に冷やせないぶん、内臓処理と水気の管理が効いてくる。管理釣り場は自宅から近いことが多いので、この3点を守れば十分に美味しく持ち帰れる。",
        ],
      },
    ],
  },
  {
    slug: "himono",
    category: "hozon",
    title: "干物の作り方｜立て塩の濃度と干し時間の目安",
    date: "2026-08-01",
    description: "釣った魚を干物にする手順を解説。立て塩と振り塩の違い、塩水の濃度と漬け時間、干し方と干し上がりの見極め、夏場の冷蔵庫干し、保存期間までまとめています。",
    gear: [
      { name: "干し網", q: "干し網 魚 三段" },
      { name: "粗塩", q: "粗塩 天日塩 1kg" },
      { name: "脱水シート", q: "ピチットシート 脱水シート" },
      { name: "バット", q: "ステンレス バット 大" },
    ],
    lead: "干物は保存食であると同時に、魚を美味しくする調理法でもある。水分が抜けて旨味が凝縮し、生のままでは出ない香りが立つ。作り方そのものは単純で、塩水に漬けて干すだけだ。仕上がりを分けるのは、塩の濃度と漬け時間、そして干し上がりの見極めの3つになる。",
    sections: [
      {
        h2: "干物に向く魚",
        body: [
          "小型から中型の、脂が乗りすぎていない魚が扱いやすい。",
          {
            list: [
              "アジ：干物の定番。20cm前後がちょうどいい",
              "イワシ：手開きにしてそのまま干せる。小型なので乾きが速い",
              "サバ：脂が多いので塩をやや強めに、干し時間は短めに",
              "カマス・キス・ハタハタ：身が薄く乾きやすい",
            ],
          },
          "逆に、身の厚い大型魚は中心まで乾くのに時間がかかり、表面だけ乾いて中が生という状態になりやすい。慣れるまでは20cm前後の魚がいい。",
          "釣れすぎて食べきれない日にこそ向く調理法でもある。冷凍と違って味が変わるので、同じ魚が続いても飽きにくい。",
        ],
      },
      {
        h2: "手順1：開く",
        body: [
          "干物にするには魚を開いて、身の断面を作る必要がある。厚みが均一になるほど乾きムラが出ない。",
          {
            list: [
              "背開き：背側から包丁を入れて開く。アジの干物はこの形が一般的",
              "腹開き：腹側から開く。サバやサンマでよく使われる",
              "手開き：包丁を使わず指で開く。イワシなど骨の柔らかい魚向き",
            ],
          },
          "どれを選んでも味は変わらない。開きやすいやり方でいい。",
          "手開きのやり方は<a href=\"/guide/iwashi-tebiraki/\">イワシの手開き</a>に図解つきでまとめてある。アジの下処理は<a href=\"/guide/aji-shitagoshirae/\">アジの下処理</a>を参照してほしい。",
          "開いたら、エラ・内臓・血合いを完全に取り除く。ここに残りがあると、干している間に傷みと臭みが出る。特に背骨に沿った血合いは、指の腹でこすってきれいに落とす。",
        ],
      },
      {
        h2: "手順2：塩をあてる",
        body: [
          "塩のあて方には、塩水に漬ける「立て塩」と、直接ふりかける「振り塩」がある。干物では立て塩をすすめたい。",
          {
            list: [
              "立て塩：塩水に漬ける。全体に均一に入る。時間で濃さを調整できる",
              "振り塩：塩を直接ふる。手軽だがムラが出やすく、部分的に塩辛くなる",
            ],
          },
          { svg: "himono-tatejio" },
          "塩水の濃度は<strong>8〜10%</strong>を基本にする。水1リットルに対して塩80〜100g。海水がおよそ3%なので、それよりかなり濃い。",
          {
            list: [
              "20cm前後のアジ・イワシ：30〜40分",
              "サバなど脂の多い魚：40〜60分（塩は10%寄りに）",
              "ごく小型の魚：20分程度",
            ],
          },
          "迷ったら短めにする。塩が薄ければ食べるときに足せるが、入れすぎた塩は抜けない。",
          { alert: "漬けたあとは真水でさっと洗い流し、すぐ水気を拭く。表面に塩水が残ったまま干すと、乾いたときに白い塩が浮いて塩辛くなる。" },
          "みりんや醤油を塩水に加えれば、みりん干しや醤油干しになる。基本の立て塩ができるようになってから試すといい。",
        ],
      },
      {
        h2: "手順3：干す",
        body: [
          { svg: "himono-hoshiami" },
          "干す条件で大事なのは、気温よりも<strong>風と湿度</strong>だ。風が当たれば表面の水分が連続して飛んでいく。逆に無風の場所では、気温が高くても乾きが進まないまま傷みだけが進む。",
          {
            steps: [
              "水気をしっかり拭く。ここが甘いと乾き始めるまでに時間がかかる。",
              "干し網やザルに、身を上（皮を下）にして並べる。",
              "魚どうしを重ねない。触れている面は乾かない。",
              "風通しのよい日陰に吊るす。直射日光は脂が酸化するので避ける。",
            ],
          },
          "干し時間の目安は、涼しい時期の屋外で半日から一晩。冬なら一晩、春秋なら3〜5時間ほどで様子を見はじめる。",
          { alert: "夏場の屋外干しは避ける。気温が高いと乾くより先に傷む。夏は後述の冷蔵庫干しに切り替える。" },
          "屋外に出す以上、虫と鳥と猫の対策は必須になる。網戸状の干し網を使えばまとめて解決する。ベランダに吊るせるタイプが扱いやすい。",
        ],
      },
      {
        h2: "干し上がりの見極め",
        body: [
          "時間ではなく、状態で判断する。",
          {
            list: [
              "表面がさらりと乾き、触っても手に水がつかない",
              "身にうっすら透明感とツヤが出る",
              "指で押すと弾力があり、身が締まっている",
              "皮がぴんと張っている",
            ],
          },
          "干しすぎるとカチカチになって、焼いてもぱさつく。「表面は乾いたが中はしっとり」が目標地点だ。一夜干しと呼ばれる状態がこれにあたる。",
          "完全に乾かした固い干物（素干し）は保存性が高いが、家庭で作る干物は一夜干しの含水量で止めておくほうが美味しく食べられる。",
        ],
      },
      {
        h2: "室内で干す・冷蔵庫で干す",
        body: [
          "ベランダがない、夏で外に出せない、虫が気になる。そういう場合は室内でも作れる。",
          {
            list: [
              "冷蔵庫干し：バットに網を敷いて魚を並べ、ラップをせず冷蔵庫へ。一晩から24時間。冷蔵庫内は低湿度なので、ゆっくり確実に乾く",
              "脱水シート：市販の脱水シートで包んで冷蔵庫に入れる。半日ほどで干物に近い状態になる",
              "扇風機：室内のザルに並べ、弱風を当てる。数時間で表面が乾く",
            ],
          },
          "特に冷蔵庫干しは、低温を保ったまま乾かせるので夏場でも失敗しない。手間も「入れて放置」だけで済む。屋外干しより気楽なので、最初の一枚はこの方法で試すのがいい。",
        ],
      },
      {
        h2: "焼き方と保存",
        body: [
          "干物は<strong>身のほうから先に焼く</strong>。身から焼いて8割ほど火を通し、返して皮目を仕上げると、身が乾きすぎず皮が香ばしくなる。",
          "生の魚より水分が少ないぶん火の通りが速い。焦げやすいので、火力は中火以下にする。",
          "保存の目安は次のとおり。",
          {
            list: [
              "冷蔵：2〜3日",
              "冷凍：1ヶ月程度",
            ],
          },
          "冷凍するときは1枚ずつラップで包み、保存袋に入れて空気を抜く。焼くときは解凍せず、凍ったまま弱めの火で焼いたほうがぱさつかない。",
          "保存の詳しい考え方は<a href=\"/guide/reitou-hozon/\">釣った魚の冷凍と保存</a>にまとめてある。",
        ],
      },
      {
        h2: "うまくいかないときは",
        body: [
          {
            list: [
              "塩辛すぎた：塩水が濃いか漬けすぎ。次回は濃度8%・時間を10分短く",
              "味がぼやける：塩が薄い。または洗いすぎ。漬けたあとの水洗いは「さっと」でよい",
              "生臭い：血合いか内臓の残り。開いたあとの掃除を丁寧に",
              "表面だけ乾いて中が生：身が厚い、または無風。開きを薄くするか風を当てる",
              "ぱさぱさになった：干しすぎ。次回は半分の時間で一度触って確認する",
              "白い粉が浮いた：塩水を洗い流していない",
            ],
          },
          "最初の数枚は、同じ魚を違う時間で干して食べ比べるのが早い。自分の家の環境に合う時間が分かれば、あとは再現するだけになる。",
        ],
      },
    ],
  },
  {
    slug: "kasago-shitagoshirae",
    title: "カサゴの下処理｜棘の外し方と煮付けまでの準備",
    date: "2026-08-01",
    fish: "カサゴ",
    description: "カサゴの下処理を、棘の落とし方から煮付け用の霜降りまで解説。硬いウロコの取り方、頭を残したままの内臓の抜き方、刺さったときの対処までまとめています。",
    gear: [
      { name: "キッチンバサミ", q: "キッチンバサミ 魚 骨" },
      { name: "ウロコ取り", q: "ウロコ取り 魚 コンパクト" },
      { name: "フィッシュグリップ", q: "フィッシュグリップ 魚つかみ" },
      { name: "出刃包丁", q: "出刃包丁 小 105mm" },
    ],
    lead: "カサゴは頭が大きく可食部が少ない魚だが、そのぶん身は締まって上品で、アラからよい出汁が出る。煮付けにすれば一匹で立派な一品になる。ただし捌く前にひとつ片付けておくことがある。全身に生えた鋭い棘だ。",
    sections: [
      {
        h2: "まず棘を警戒する",
        body: [
          "カサゴは背びれ・腹びれ・尻びれ・エラ蓋に、針のように硬く尖った棘を持っている。素手でつかむと簡単に刺さる。",
          { svg: "kasago-toge" },
          "カサゴの棘そのものに毒はないとされる。それでも刺さると深く入り、傷口から細菌が入って腫れることがある。海の細菌による感染は治りが悪いことがあるので、軽く見ないほうがいい。",
          { alert: "同じ根魚でも、ハオコゼ・アイゴ・ゴンズイ・ミノカサゴなどには毒がある。釣れた魚の種類に確信が持てないときは、素手で触らずフィッシュグリップを使う。" },
          "扱うときは次のようにする。",
          {
            list: [
              "フィッシュグリップかタオルで持つ",
              "掴むなら下あごか、背びれを寝かせて背中側から包み込むように持つ",
              "刺さったら傷口を絞って血を出し、流水でよく洗う。腫れや熱を持つようなら受診する",
            ],
          },
        ],
      },
      {
        h2: "釣り場でやること",
        body: [
          "カサゴは生命力が強く、クーラーボックスに入れてもしばらく生きている。逆にいえば、締めずに放置すると長く暴れて身が疲れる。",
          {
            steps: [
              "釣れたらエラの付け根に刃先を入れて血を抜く。",
              "海水と氷を混ぜた潮氷で冷やす。",
            ],
          },
          "潮氷の作り方や締め方は<a href=\"/guide/mochikaeri/\">釣った魚の持ち帰り方</a>にまとめてある。",
          "カサゴは傷みにくい魚なので、青魚ほど神経質にならなくてよい。ただし冷やすこと自体は味に効く。",
        ],
      },
      {
        h2: "手順1：棘を切り落とす",
        body: [
          "台所での作業を安全にするため、捌く前にキッチンバサミで棘を落としてしまう。これをやるかどうかで作業の緊張感がまったく違う。",
          {
            steps: [
              "背びれを根元から切り取る。前半分の棘が特に硬い。",
              "腹びれ・尻びれも切る。",
              "胸びれは料理の見た目に関わるので、煮付けにするなら残してもよい。",
            ],
          },
          "煮付けで姿のまま出したい場合は、背びれだけ切っておいて残りは残す、という折衷でもいい。棘は加熱しても柔らかくならないので、食べるときの安全も考えて判断する。",
        ],
      },
      {
        h2: "手順2：ウロコを取る",
        body: [
          "カサゴのウロコは小さく硬く、体にしっかり張り付いている。アジのように簡単には落ちない。",
          {
            list: [
              "ウロコ取りを使い、尾から頭へ向かって細かく動かす",
              "頭の上、エラ蓋の周り、ヒレの根元は取り残しやすい",
              "流水をあてながらやると飛び散らない",
            ],
          },
          "煮付けにするとウロコの残りが口に当たるので、指で撫でてざらつきがなくなるまで確認する。頭も食べる魚なので、頭部のウロコこそ丁寧にやる価値がある。",
        ],
      },
      {
        h2: "手順3：エラと内臓を出す",
        body: [
          "煮付けにするなら頭は落とさない。頭を残したまま、エラと内臓だけを抜く。",
          {
            steps: [
              "エラ蓋を持ち上げ、エラの上下の付け根を包丁の先で切り離す。",
              "エラをつまんで引き抜く。内臓とつながっているので、ゆっくり引くと一緒に出てくることがある。",
              "腹を肛門から胸びれの手前まで切り開き、残った内臓をかき出す。",
              "背骨に沿った血合いを、包丁の先や指で掻き出す。",
            ],
          },
          "唐揚げにするなら頭を落としてしまってもいい。その場合は胸びれの後ろから斜めに包丁を入れる。落とした頭は割って味噌汁に使える。",
          "内臓と血合いは臭みの元になる。ここを丁寧にやるかどうかが、煮付けの仕上がりを分ける。",
        ],
      },
      {
        h2: "手順4：洗って水気を拭く",
        body: [
          "腹の中を流水でよく洗う。歯ブラシがあると、背骨まわりの血合いが落としやすい。",
          "洗い終わったら、キッチンペーパーで表面と腹の中の水気を拭き取る。",
          { alert: "水気が残っていると、煮付けの味が薄まり、揚げれば油がはねる。ここを省略しない。" },
        ],
      },
      {
        h2: "煮付けの前に霜降りをする",
        body: [
          { svg: "kasago-shimofuri" },
          "カサゴを煮付けにするなら、湯にくぐらせる「霜降り」をひと手間かけると仕上がりが変わる。",
          {
            steps: [
              "魚をバットに並べ、上から熱湯を回しかける。表面が白くなればよい。",
              "すぐに冷水に取る。",
              "指でこすりながら、残ったウロコ・ぬめり・血の塊を洗い落とす。",
              "水気を拭く。",
            ],
          },
          "こうすると表面のぬめりと臭みが落ち、煮汁が濁らない。仕上がりの味が澄む。",
          "熱湯を長くあてると身に火が入りすぎるので、かけたらすぐ冷水に落とすのがコツになる。",
        ],
      },
      {
        h2: "向いている料理",
        body: [
          {
            list: [
              "煮付け：定番。頭ごと煮て、頬肉まで食べる",
              "唐揚げ：小型ならまるごと。骨まで食べられる",
              "味噌汁・潮汁：アラから濃い出汁が出る",
              "刺身：25cm以上の良型なら。身が締まって歯ごたえがある",
            ],
          },
          "小型が数匹釣れたときは、まとめて唐揚げにするのが手っ取り早い。頭も骨もそのまま食べられる。",
          "カサゴのレシピは<a href=\"/fish/kasago/\">カサゴのページ</a>にまとめてある。",
        ],
      },
      {
        h2: "保存",
        body: [
          "カサゴは白身で傷みにくいほうだが、内臓を抜いていない状態では話が別になる。",
          {
            list: [
              "当日〜翌日：下処理を済ませ、水気を拭いてラップで包み冷蔵",
              "数日以上：下処理後に水気を拭き、ラップ＋保存袋で冷凍（3〜4週間）",
            ],
          },
          "冷凍のやり方は<a href=\"/guide/reitou-hozon/\">釣った魚の冷凍と保存</a>にまとめてある。頭付きのまま冷凍するとかさばるので、煮付け用に1匹ずつ包むか、頭を落として身だけにするか決めてから凍らせる。",
        ],
      },
    ],
  },
  {
    slug: "kisu-shitagoshirae",
    title: "キスの下処理と背開き｜天ぷらのための一手間",
    date: "2026-08-01",
    fish: "キス",
    description: "キスの下処理と背開きの手順を解説。細かいウロコの取り方、背開きで中骨を外す順番、天ぷらで油がはねない下準備、塩焼きや南蛮漬けへの使い分けまでまとめています。",
    gear: [
      { name: "小出刃包丁", q: "出刃包丁 小 105mm" },
      { name: "骨抜き", q: "骨抜き ステンレス 魚" },
      { name: "キッチンペーパー", q: "キッチンペーパー 厚手" },
      { name: "天ぷら鍋", q: "天ぷら鍋 温度計付き" },
    ],
    lead: "キスは投げ釣りで数が釣れる魚で、天ぷらにすると身がふわりと膨らむ。淡白で骨も柔らかく、下処理そのものは難しくない。ただし身が薄く柔らかいぶん、包丁の入れ方と水気の扱いで仕上がりがはっきり変わる。",
    sections: [
      {
        h2: "釣り場でやること",
        body: [
          "キスは20cm前後の小型魚なので、氷締めで十分だ。海水と氷を混ぜた潮氷に入れて一気に冷やす。",
          "数が釣れる魚なので、クーラーボックスに詰め込みすぎないよう気をつけたい。魚どうしが重なった中心部は冷えが遅れる。",
          { alert: "キスは身が柔らかく、鮮度が落ちると一気に水っぽくなる。釣った日に処理するのが基本。" },
          "潮氷の作り方は<a href=\"/guide/mochikaeri/\">釣った魚の持ち帰り方</a>にまとめてある。",
        ],
      },
      {
        h2: "手順1：ウロコを取る",
        body: [
          "キスのウロコは薄くて細かく、力を入れなくても落ちる。包丁の背を尾から頭へ向かって軽く滑らせるだけでいい。",
          "身が柔らかいので、力を入れると皮ごと削れてしまう。ウロコ取りより包丁の背のほうが扱いやすい魚だ。",
          "腹側とエラ蓋の周りに残りやすいので、指で撫でて確認する。",
        ],
      },
      {
        h2: "手順2：頭とワタを落とす",
        body: [
          "天ぷらや南蛮漬けにするなら頭を落とす。塩焼きにするなら頭を残して腹だけ抜く。",
          {
            steps: [
              "胸びれの後ろから、頭に向かって斜めに包丁を入れる。",
              "裏返して同じ角度で入れ、頭を落とす。",
              "腹の切り口から内臓を指か包丁の先でかき出す。",
              "背骨に沿った血合いを掻き出す。",
            ],
          },
          "キスの内臓は小さくまとまっているので、頭を落とすときに一緒に引き出せることも多い。",
        ],
      },
      {
        h2: "手順3：背開きにする",
        body: [
          "天ぷらにするなら背開きだ。身が薄いので、開くと火の通りが均一になる。",
          { svg: "kisu-segaki" },
          {
            steps: [
              "頭を落とした側を右、背を手前にして置く。",
              "背側から中骨の上に沿って、浅く切り込みを入れる。",
              "同じ線をなぞって少しずつ深くし、尾の付け根まで開く。",
              "腹の皮は切り離さない。ここでつながったまま、観音開きのように開く。",
            ],
          },
          "一度で深く切ろうとしないのがコツになる。浅い切り込みを3回繰り返すほうが、身を割らずに済む。",
          "腹側から開く腹開きでもよいが、天ぷらでは背開きのほうが形が安定する。",
        ],
      },
      {
        h2: "手順4：中骨を外す",
        body: [
          { svg: "kisu-nakabone" },
          {
            steps: [
              "開いた身を広げ、中骨の下に包丁を入れる。",
              "包丁を寝かせ、骨に沿って尾のほうへ滑らせる。",
              "尾の手前まで来たら、中骨を持ち上げて切り離す。",
              "腹骨が残っていれば薄く削ぎ取る。",
            ],
          },
          "キスの骨は柔らかいので、多少残っても天ぷらなら気にならない。神経質に取るより、身を割らないことを優先したい。",
          "小骨が気になる場合は、骨抜きで血合い骨を抜く。ただし身が柔らかいので、引っ張るのではなく、押さえながら真上へ抜く。",
        ],
      },
      {
        h2: "天ぷらにする前の下準備",
        body: [
          "ここが一番効く工程になる。キスは水分の多い魚なので、水気を残したまま揚げると油が激しくはねる。",
          {
            steps: [
              "開いた身をキッチンペーパーで挟み、両面の水気をしっかり取る。",
              "尾を残す場合、尾の中に水分が溜まっているので、包丁の背でしごいて押し出す。",
              "尾の先端を少し切り落とす。ここに残った水分がはねの原因になる。",
              "揚げる直前まで冷蔵庫で冷やしておく。",
            ],
          },
          { alert: "尾の水分を抜かずに揚げると、高温の油が跳ねて危ない。天ぷらでは省略しない工程。" },
          "衣は薄く。キスの身は繊細なので、厚い衣だと魚の味が隠れてしまう。",
        ],
      },
      {
        h2: "料理別の使い分け",
        body: [
          {
            list: [
              "天ぷら：背開き。定番中の定番",
              "塩焼き：頭を残してワタだけ抜く。20cm以上の良型向き",
              "フライ：背開き。パン粉をつけて揚げる",
              "南蛮漬け：頭を落として二度揚げし、骨ごと食べる",
              "刺身：ごく新鮮な良型のみ。皮を引いて薄造りに",
            ],
          },
          "小型が数釣れたときは、頭を落として丸ごと南蛮漬けにするのが後片付けまで含めて楽だ。",
          "キスのレシピは<a href=\"/fish/kisu/\">キスのページ</a>にまとめてある。",
        ],
      },
      {
        h2: "保存",
        body: [
          "キスは水分が多く、身が柔らかい。冷蔵で寝かせるほど水っぽくなるので、釣った日か翌日までに食べきるのが基本になる。",
          {
            list: [
              "当日：下処理して水気を拭き、ラップで包み冷蔵",
              "翌日以降：背開きにして水気を拭き、1回分ずつラップ＋保存袋で冷凍（3〜4週間）",
              "干物にする：開いて塩水に漬け、一晩干す",
            ],
          },
          "冷凍する場合も、開いてからのほうが解凍後に扱いやすい。詳しくは<a href=\"/guide/reitou-hozon/\">釣った魚の冷凍と保存</a>を参照してほしい。",
          "身が薄く乾きやすいので、干物にも向いている。作り方は<a href=\"/guide/himono/\">干物の作り方</a>にまとめてある。",
        ],
      },
    ],
  },
  {
    slug: "tai-shitagoshirae",
    title: "タイの下処理｜硬いウロコの取り方と兜割り",
    date: "2026-08-01",
    fish: "タイ",
    description: "タイの下処理を、飛び散る硬いウロコの取り方から兜割り、三枚おろしまで解説。すき引きのやり方、松皮造りのための湯引き、アラの使い道までまとめています。",
    gear: [
      { name: "ウロコ取り", q: "ウロコ取り 大型魚" },
      { name: "出刃包丁", q: "出刃包丁 165mm" },
      { name: "骨抜き", q: "骨抜き ステンレス 魚" },
      { name: "大型まな板", q: "まな板 大きい 魚用" },
    ],
    lead: "タイは姿も味も申し分ない魚だが、下処理の難易度はアジやイワシとは別物になる。ウロコは硬くて盛大に飛び散り、骨も硬い。逆にいえば、この2つに対処法さえ持っていれば、あとは他の魚と同じだ。",
    sections: [
      {
        h2: "タイが手ごわい理由",
        body: [
          "タイの下処理でつまずくのは、たいてい次の3点になる。",
          {
            list: [
              "ウロコが硬く大きく、力を入れると台所じゅうに飛び散る",
              "骨が硬く、普通の包丁では頭が割れない",
              "体高があるので、まな板と包丁のサイズが足りないことがある",
            ],
          },
          "つまり技術より段取りの問題が大きい。ウロコの飛び散りをどう抑えるか、頭をどう割るか。この2つを先に決めておけば作業は速い。",
        ],
      },
      {
        h2: "釣り場でやること",
        body: [
          {
            steps: [
              "釣れたらエラの付け根に刃を入れて血を抜く。タイは血が多く、残すと生臭さが出る。",
              "海水を張ったバケツに数分入れて血を抜き切る。",
              "潮氷に入れて冷やす。体高があるので、魚が完全に浸かる量の潮氷を作る。",
            ],
          },
          { alert: "刺身にするつもりなら血抜きは省略しない。加熱調理でも、血が残った身は臭みが出る。" },
          "潮氷の作り方や血抜きの位置は<a href=\"/guide/mochikaeri/\">釣った魚の持ち帰り方</a>に図解つきでまとめてある。",
        ],
      },
      {
        h2: "手順1：ウロコを取る",
        body: [
          "タイのウロコは硬く、勢いよく飛ぶ。無策でやると壁・床・服にまで散る。対策は3つある。",
          { svg: "tai-uroko" },
          {
            list: [
              "ポリ袋の中で取る：大きめの袋に魚を入れ、袋の中に手を入れて作業する。いちばん手軽で効果が高い",
              "水を張ったボウルやシンクの中で取る：水がウロコを受け止める。飛ばないが手元は見えにくい",
              "すき引き：包丁で皮の表面を薄く削ぐ。飛び散らず仕上がりも良いが、慣れが要る",
            ],
          },
          "家庭でやるなら袋の中が現実的だ。新聞紙を敷くだけでは飛散は止まらない。",
          "取り残しやすいのは、頭の周り、腹びれの根元、背びれの際、尾の付け根。指で撫でてざらつきを確かめる。皮ごと食べる料理にするなら特に丁寧にやる。",
        ],
      },
      {
        h2: "手順2：エラと内臓を出す",
        body: [
          {
            steps: [
              "エラ蓋を持ち上げ、エラの上下の付け根を切り離して引き抜く。",
              "腹を肛門から頭側へ切り開く。",
              "内臓を取り出す。",
              "背骨に沿った血合いを、包丁の先や指で掻き出す。",
              "腹の中を流水でよく洗い、水気を拭く。",
            ],
          },
          "タイの内臓は他の魚より量が多い。血合いも厚いので、歯ブラシがあると掻き出しやすい。",
        ],
      },
      {
        h2: "手順3：頭を落とし、兜に割る",
        body: [
          "頭は捨てずに割って使う。兜煮や潮汁にすると、身より濃い旨味が出る。",
          { svg: "tai-kabutowari" },
          {
            steps: [
              "胸びれの後ろから斜めに包丁を入れ、裏返して同じ角度で入れて頭を落とす。",
              "頭を立てて置き、上あごの中心（前歯の間）に出刃の刃元を当てる。",
              "包丁の背を手のひらか木槌で叩き、少しずつ押し進める。",
              "硬いところで止まったら、包丁を抜いて角度を変えて入れ直す。",
            ],
          },
          { alert: "無理に力任せで叩かない。刃が滑って手を切る。硬いと感じたら一度抜いて、位置を少し変えて入れ直すほうが安全で速い。" },
          "出刃がない場合は、頭を割らずにそのまま煮ることもできる。頬肉と目の周りは割らなくても食べられる。",
          "割った頭は、エラの残りを取ってから塩をふり、しばらく置いてから熱湯をかけて霜降りにすると臭みが抜ける。",
        ],
      },
      {
        h2: "手順4：三枚におろす",
        body: [
          {
            steps: [
              "頭側を右、背を手前にして置き、背びれの上から背骨に沿って浅く切り込みを入れる。",
              "同じ線をなぞりながら、少しずつ深く切り進める。",
              "腹側も同様に、腹びれの下から切り込む。",
              "背骨の上を通して包丁を差し込み、身を離す。",
              "裏返して同じ手順で二枚目を取る。",
            ],
          },
          "体高があるので、一度で切り離そうとしないことが重要になる。背側と腹側から別々に切り進め、最後に中央でつなげる。",
          "腹骨は包丁を寝かせて薄く削ぎ取る。血合い骨は骨抜きで抜くか、細く切り取る。タイの血合い骨は太いので、骨抜きより「切り取る」ほうが確実なことが多い。",
        ],
      },
      {
        h2: "皮を活かす：松皮造り",
        body: [
          "タイの皮は旨味があり、刺身でも皮つきで食べられる。ただし生の皮は硬いので、湯をかけて柔らかくする。",
          {
            steps: [
              "皮つきの柵をまな板に置き、上から布巾をかぶせる。",
              "熱湯を回しかける。",
              "すぐに氷水に落とす。",
              "水気を拭き、皮目に細かく切り込みを入れてから切る。",
            ],
          },
          "皮の表面が松の樹皮のように見えることから松皮造りと呼ばれる。皮の香りと身の甘みが同時に来るので、新鮮なタイが手に入ったら一度は試す価値がある。",
        ],
      },
      {
        h2: "アラを捨てない",
        body: [
          "タイは可食部の割合が高くない魚だが、アラの価値が高い。",
          {
            list: [
              "頭：兜煮、潮汁",
              "カマ：塩焼き",
              "中骨：潮汁、タイめしの出汁",
              "腹身の薄い部分：塩焼き、みそ汁",
            ],
          },
          "アラは使う前に必ず霜降りをする。塩をふって20分ほど置き、熱湯をかけてから冷水で洗う。これで血の塊とぬめりが落ち、汁が濁らない。",
          "タイのレシピは<a href=\"/fish/tai/\">タイのページ</a>にまとめてある。",
        ],
      },
      {
        h2: "保存",
        body: [
          {
            list: [
              "当日：柵のまま、キッチンペーパーで包んで冷蔵。刺身は切る直前まで柵で置く",
              "翌日：ペーパーを取り替えて冷蔵。白身は1〜2日置くと旨味が増す",
              "数日以上：柵を1回分ずつラップで包み、保存袋に入れて冷凍（3〜4週間）",
            ],
          },
          "タイのような白身は、釣った当日より翌日のほうが旨味が乗る。当日は歯ごたえ、翌日は旨味と考えて食べ分けるといい。",
          "冷凍のやり方は<a href=\"/guide/reitou-hozon/\">釣った魚の冷凍と保存</a>にまとめてある。",
        ],
      },
    ],
  },
  {
    slug: "buri-shitagoshirae",
    title: "ブリ・ハマチの下処理｜大型魚のさばき方と部位の分け方",
    date: "2026-08-01",
    fish: "ブリ",
    description: "ブリ・ハマチの下処理を解説。大型魚の血抜きと持ち帰り、部位の分け方、背身と腹身の使い分け、ブリ糸状虫の正体と対処、熟成の考え方までまとめています。",
    gear: [
      { name: "出刃包丁（大）", q: "出刃包丁 180mm" },
      { name: "大型まな板", q: "まな板 特大 業務用" },
      { name: "フィッシュグリップ", q: "フィッシュグリップ 大型魚" },
      { name: "大型クーラーボックス", q: "クーラーボックス 釣り 35L" },
    ],
    lead: "ブリやハマチが釣れると嬉しいが、家に帰ってから途方に暮れることがある。60cmを超える魚は、まな板からはみ出し、包丁が骨に負ける。大型魚の下処理は技術というより準備の問題で、道具と場所が揃えばあとは小型魚と同じ手順の繰り返しになる。",
    sections: [
      {
        h2: "先に道具と場所を用意する",
        body: [
          "包丁が小さいと、背骨に当たったところで進まなくなる。まな板が小さいと、魚が滑って危ない。",
          {
            list: [
              "出刃包丁：刃渡り165〜180mmは欲しい。三徳包丁では骨に負ける",
              "まな板：魚が収まるサイズ。無ければシンクに板を渡す、段ボールにラップを巻いて代用するなど",
              "キッチンペーパー：大量に使う。血と水気を拭き続けることになる",
              "ゴミ袋：アラと内臓の量が多い。先に用意しておく",
            ],
          },
          "作業を始める前に流し台を空にしておくと、途中で手が止まらない。大型魚は工程が長いので、段取りが仕上がりに効いてくる。",
        ],
      },
      {
        h2: "釣り場でやること",
        body: [
          "青物は暴れる時間が長いほど身に熱と乳酸がこもり、身が焼けて味が落ちる。釣れたらすぐ処理する。",
          {
            steps: [
              "エラの付け根と尾の付け根に刃を入れ、海水を張ったバケツで血を抜く。",
              "血が抜けたら、大型クーラーの潮氷に入れる。魚が入りきらない場合は、袋に入れて氷を当てる。",
              "クーラーは日陰に置き、開け閉めを減らす。",
            ],
          },
          { alert: "ブリは体温が上がりやすく、締めずに置くと身が白く濁る「身焼け」を起こす。こうなると刺身には向かなくなる。" },
          "潮氷の作り方は<a href=\"/guide/mochikaeri/\">釣った魚の持ち帰り方</a>にまとめてある。",
        ],
      },
      {
        h2: "手順1：ウロコを取る",
        body: [
          "ブリのウロコは細かく、皮に張り付いている。タイのように飛び散らないので作業自体は楽だ。",
          "ウロコ取りか包丁の背で、尾から頭へ向かって落とす。ぬめりが強いので、先に塩をふって水で流すと作業しやすい。",
          "皮を引いて刺身にするなら、ウロコ取りは軽くて構わない。皮ごと焼くなら丁寧に落とす。",
        ],
      },
      {
        h2: "手順2：頭とカマを落とす",
        body: [
          { svg: "buri-bui" },
          "大型魚は最初に大きく分解してしまうと、その後の取り回しが一気に楽になる。",
          {
            steps: [
              "胸びれと腹びれを頭側に含めるように、斜めに包丁を入れる。",
              "裏返して同じ角度で入れ、背骨まで達したら包丁の刃元で押し切る。",
              "落とした頭を縦に割り、カマ（胸びれの付け根の身）を切り分ける。",
            ],
          },
          "カマは焼くと脂が乗っていて美味しい部位で、捨てるのはもったいない。頭を落とすときにカマを頭側に付けておくと、あとで分けやすい。",
          "背骨が硬くて切れないときは、無理をせず包丁を抜き、少し位置をずらして入れ直す。刃をこじると欠ける。",
        ],
      },
      {
        h2: "手順3：内臓を出す",
        body: [
          {
            steps: [
              "腹を肛門から頭側へ切り開く。",
              "内臓を取り出す。量が多いので、先にゴミ袋を口を開けて置いておく。",
              "背骨に沿った血合いを掻き出す。ブリの血合いは厚く、色も濃い。",
              "腹の中を流水で洗い、水気を拭く。",
            ],
          },
          { alert: "内臓を洗った水が身にかかり続けないよう気をつける。大型魚は作業時間が長いので、身が水を吸いやすい。" },
        ],
      },
      {
        h2: "手順4：三枚におろす",
        body: [
          "小型魚のように一気に切ろうとしないのがコツになる。背側と腹側から別々に切り進める。",
          {
            steps: [
              "背を手前にして置き、背びれの上に沿って浅く切り込む。",
              "同じ線を3〜4回なぞり、背骨に達するまで深くする。",
              "魚を回して腹側を手前にし、腹びれの下から同じように切り進める。",
              "背骨の上に包丁を寝かせて差し入れ、尾に向かって滑らせて片身を外す。",
              "裏返して同じ手順で二枚目を取る。",
            ],
          },
          "包丁を長く使い、引きながら切る。押して切ろうとすると身が潰れる。",
          "おろした身は、背身と腹身に分けてから腹骨を削ぐ。大きい魚は柵取りまで済ませてしまうと、保存も調理も扱いやすくなる。",
        ],
      },
      {
        h2: "部位を使い分ける",
        body: [
          {
            list: [
              "背身：脂が控えめで身が締まっている。刺身、しゃぶしゃぶ向き",
              "腹身（ハラス）：脂が多い。塩焼き、照り焼き向き",
              "カマ：焼き物の定番。塩をふって焼くだけでいい",
              "アラ（中骨・頭）：アラ汁、ブリ大根",
              "血合い：加熱して食べる。生姜と一緒に煮ると臭みが出ない",
            ],
          },
          "同じ魚でも部位で脂の量がまったく違うので、全部を同じ料理に使うのはもったいない。刺身と焼き物と汁物に分けると、一匹で三日分の献立になる。",
          "ブリのレシピは<a href=\"/fish/buri/\">ブリのページ</a>、ハマチは<a href=\"/fish/hamachi/\">ハマチのページ</a>にまとめてある。",
        ],
      },
      {
        h2: "ブリ糸状虫を知っておく",
        body: [
          { svg: "buri-shijouchuu" },
          "ブリをおろすと、血合いのあたりに赤い糸のようなものが入っていることがある。長さは数cmから数十cmになる。",
          "これはブリ糸状虫で、ブリ類に寄生する線虫だ。名前も見た目も不安になるが、<strong>人には寄生せず、食べても害はない</strong>とされている。加熱や冷凍で処理する必要もない。",
          "ただし見た目が良くないので、見つけたら箸やピンセットで引き出して取り除く。身の色に紛れているので、血合い付近を明るい場所で確認するとよい。",
          { alert: "ブリ糸状虫とは別に、ブリにもアニサキスは寄生する。こちらは人に症状を起こす。生食する場合は内臓を早く抜き、身を目視で確認する。" },
          "アニサキスの見分け方と対処は<a href=\"/guide/saba-shitagoshirae-anisakis/\">サバの下処理とアニサキス対策</a>にまとめてある。白く不透明で渦を巻いているのがアニサキス、赤くて細長いのがブリ糸状虫、と覚えておくと現場で迷わない。",
        ],
      },
      {
        h2: "寝かせて食べる",
        body: [
          "ブリのような大型の青物は、釣った当日より1〜2日置いたほうが旨味が出る。死後硬直が解けて、旨味成分が増えるためだ。",
          {
            steps: [
              "柵に取り、水気をよく拭く。",
              "キッチンペーパーで包み、その上からラップで包む。",
              "冷蔵庫のチルド室に入れる。",
              "1日ごとにペーパーを取り替える。",
            ],
          },
          "ペーパーを替えるのは、出てきた水分（ドリップ）を身に戻さないためだ。これを怠ると生臭くなる。",
          { alert: "寝かせるのは、血抜きと冷却がきちんとできている魚に限る。処理が甘い魚を置くと、熟成ではなく単に傷む。" },
          "当日は歯ごたえ、翌日は旨味。同じ魚で味が変わるのを確かめられるのは、釣った人の特権といっていい。",
        ],
      },
      {
        h2: "保存",
        body: [
          {
            list: [
              "冷蔵：柵のまま2〜3日（血抜きが済んでいる場合）",
              "冷凍：1回分ずつ柵で包んで2〜3週間。脂が多いぶん白身より短い",
              "アラ：早めに使う。冷凍するなら霜降りしてから",
            ],
          },
          "一匹が大きいので、最初にどこまで食べてどこから冷凍するかを決めてから作業すると無駄がない。詳しくは<a href=\"/guide/reitou-hozon/\">釣った魚の冷凍と保存</a>を参照してほしい。",
        ],
      },
    ],
  },
  {
    slug: "suzuki-shitagoshirae",
    title: "スズキの下処理｜皮の引き方と磯臭さの抜き方",
    date: "2026-08-01",
    fish: "スズキ",
    description: "スズキ（シーバス）の下処理を解説。エラ蓋の鋭さへの注意、硬い皮の引き方、釣った場所によって出る臭みの抜き方、洗いのやり方、部位の使い分けまでまとめています。",
    gear: [
      { name: "出刃包丁", q: "出刃包丁 165mm" },
      { name: "柳刃包丁", q: "柳刃包丁 210mm" },
      { name: "フィッシュグリップ", q: "フィッシュグリップ 大型魚" },
      { name: "骨抜き", q: "骨抜き ステンレス 魚" },
    ],
    lead: "スズキは身近な大物で、60cmを超える個体も珍しくない。上品な白身で刺身にもフライにも向くが、二つだけ気をつける点がある。エラ蓋がカミソリのように鋭いことと、釣れた場所によって臭みが出ることだ。",
    sections: [
      {
        h2: "エラ蓋で手を切らない",
        body: [
          { alert: "スズキのエラ蓋の縁は非常に鋭く、素手でつかむと簡単に切れる。釣り場でも台所でも、掴む位置に注意する。" },
          "掴むときは下あごを持つか、フィッシュグリップを使う。エラ蓋に指を差し込むのは避けたい。",
          "背びれにも棘があり、こちらも刺さると痛い。カサゴほど鋭くはないが、暴れているときは油断できない。棘の扱いについては<a href=\"/guide/kasago-shitagoshirae/\">カサゴの下処理</a>にまとめてある。",
        ],
      },
      {
        h2: "釣り場でやること",
        body: [
          {
            steps: [
              "エラの付け根に刃を入れて血を抜く。スズキは血が多く、白身に血が回ると台無しになる。",
              "海水を張ったバケツで数分血を抜き、潮氷に入れる。",
              "大型で入りきらない場合は、袋に入れてから氷を当てる。",
            ],
          },
          "白身は血の色が目立つので、血抜きの丁寧さがそのまま見た目に出る。刺身にするつもりなら、ここは省略できない。",
          "潮氷の作り方は<a href=\"/guide/mochikaeri/\">釣った魚の持ち帰り方</a>にまとめてある。",
        ],
      },
      {
        h2: "臭みは釣り場で決まる",
        body: [
          "スズキは河口や港湾など、水質の影響を受ける場所でよく釣れる。同じ魚でも、釣れた場所によって臭いの強さが変わる。",
          {
            list: [
              "外洋や潮通しのよい場所の個体：臭みはほとんど出ない。刺身向き",
              "河口・汽水域の個体：やや泥臭さが出ることがある",
              "港湾の停滞した水域の個体：臭みが強く出ることがある。加熱調理向き",
            ],
          },
          "臭みが気になる個体は、血抜きと皮の処理を丁寧にやると軽減できる。臭いの多くは血、ぬめり、皮に由来するためだ。",
          {
            steps: [
              "血抜きを釣り場で確実にやる。",
              "捌く前に、体表のぬめりを塩でこすって洗い流す。",
              "皮を引いて調理する。",
              "それでも気になるなら、酒や生姜を使う料理（ムニエル、アクアパッツァ、ソテー）にする。",
            ],
          },
          "刺身にするなら、臭みの少ない個体を選ぶのがいちばん確実だ。無理に生食にこだわらず、加熱調理に回すという判断も味のうちになる。",
        ],
      },
      {
        h2: "手順1：ウロコとぬめりを取る",
        body: [
          "スズキのウロコは中くらいの硬さで、ウロコ取りで問題なく落ちる。タイほど飛び散らない。",
          "先にぬめりを処理しておくと作業が楽になる。塩を全体にふって手でこすり、流水で洗い流す。ぬめりが取れると包丁も滑りにくくなる。",
          "取り残しやすいのは、頭の周り、背びれの際、腹びれの根元。",
        ],
      },
      {
        h2: "手順2：頭と内臓",
        body: [
          {
            steps: [
              "胸びれの後ろから斜めに包丁を入れ、裏返して同じ角度で入れて頭を落とす。",
              "腹を肛門から切り開き、内臓を取り出す。",
              "背骨に沿った血合いを掻き出す。",
              "腹の中を流水で洗い、水気を拭く。",
            ],
          },
          "頭はアラ汁や潮汁に使える。スズキの頭は大きく、出汁がよく出る。使う前に霜降り（塩をふって熱湯をかけ、冷水で洗う）をしておくと臭みが残らない。",
        ],
      },
      {
        h2: "手順3：三枚におろす",
        body: [
          "大型なので、背側と腹側から別々に切り進める。手順はブリと同じだ。",
          {
            steps: [
              "背を手前にして、背びれの上から背骨に沿って浅く切り込む。",
              "同じ線を3〜4回なぞって深くする。",
              "腹側からも同様に切り進める。",
              "背骨の上に包丁を寝かせて差し入れ、身を外す。",
              "裏返して二枚目を取る。",
            ],
          },
          "大型魚の三枚おろしの詳しい進め方は<a href=\"/guide/buri-shitagoshirae/\">ブリ・ハマチの下処理</a>にまとめてある。",
          "腹骨を薄く削ぎ、血合い骨を骨抜きで抜けば柵の完成になる。",
        ],
      },
      {
        h2: "手順4：皮を引く",
        body: [
          "スズキの皮は厚くて丈夫で、そのぶん引きやすい。刺身にするなら必ず引く。",
          { svg: "suzuki-kawahiki" },
          {
            steps: [
              "柵を皮目を下にして、まな板の手前側に置く。尾側を左に向ける。",
              "尾側の端の皮と身の間に、包丁の先を少し入れる。",
              "出た皮の端を左手でしっかり持つ。",
              "包丁を寝かせてまな板に押しつけるようにし、皮を左手で引っぱりながら包丁を前へ滑らせる。",
            ],
          },
          "動かすのは包丁ではなく皮のほうだ、と考えるとうまくいく。包丁は角度を保ったまま、ほとんど動かさない。",
          { alert: "包丁の刃を上に向けると身を削ってしまう。刃はまな板に向けて寝かせる。" },
          "引いた皮は捨てずに、湯引きして刻んで和え物にしたり、パリパリに焼いて食べることもできる。",
        ],
      },
      {
        h2: "洗いにする",
        body: [
          "夏場のスズキは「洗い」にすると身が締まって美味しい。氷水で身を締める刺身の食べ方だ。",
          {
            steps: [
              "皮を引いた柵を、薄めのそぎ切りにする。",
              "氷水にさっとくぐらせる。長く浸けると旨味が抜ける。",
              "身がくるりと縮んだらすぐ引き上げ、水気をしっかり拭く。",
              "酢味噌やポン酢で食べる。",
            ],
          },
          "脂の乗った魚には向かないが、スズキのような淡白な白身にはよく合う。臭みがやや気になる個体でも、洗いにすると食べやすくなる。",
        ],
      },
      {
        h2: "保存",
        body: [
          {
            list: [
              "当日：柵のままキッチンペーパーで包んで冷蔵",
              "翌日以降：ペーパーを取り替えて冷蔵。白身なので1〜2日置くと旨味が乗る",
              "冷凍：柵を1回分ずつラップで包み、保存袋へ（3〜4週間）",
            ],
          },
          "大型で一度に食べきれないので、最初に刺身用・焼き用・汁物用を分けてしまうと扱いやすい。",
          "スズキのレシピは<a href=\"/fish/suzuki/\">スズキのページ</a>、冷凍の詳細は<a href=\"/guide/reitou-hozon/\">釣った魚の冷凍と保存</a>にまとめてある。",
        ],
      },
    ],
  },
  {
    slug: "nijimasu-shitagoshirae",
    title: "ニジマスの下処理｜ぬめりの落とし方と小骨の扱い",
    date: "2026-08-01",
    fish: "ニジマス",
    description: "ニジマスの下処理を解説。臭みの元になるぬめりの落とし方、内臓と血合いの取り方、塩焼き・ムニエルへの使い分け、淡水魚を生食しない理由までまとめています。",
    gear: [
      { name: "小出刃包丁", q: "出刃包丁 小 105mm" },
      { name: "ウロコ取り", q: "ウロコ取り 魚 コンパクト" },
      { name: "キッチンペーパー", q: "キッチンペーパー 厚手" },
      { name: "粗塩", q: "粗塩 天日塩 1kg" },
    ],
    lead: "管理釣り場で釣れるニジマスは、下処理そのものは簡単な部類に入る。ウロコは細かく、骨は柔らかく、内臓もまとまって取れる。押さえるべきは体表のぬめりと、腹の中の血合い。この2つが臭みのほぼすべてを決める。",
    sections: [
      {
        h2: "ぬめりが臭みの正体",
        body: [
          "ニジマスに「泥臭い」「川魚の臭いがする」という印象を持つ人は多いが、その原因の大半は体表のぬめりだ。身そのものは淡白で癖がない。",
          "つまり、ぬめりさえきちんと落とせば臭みはほとんど気にならなくなる。焼く前にどれだけ丁寧に洗ったかが、そのまま仕上がりに出る。",
          { svg: "nijimasu-numeri" },
          {
            steps: [
              "全体に粗塩をふり、手でこすり洗いする。",
              "ぬめりが白く浮いてきたら、流水で洗い流す。",
              "ざらつきが残っていたらもう一度繰り返す。",
            ],
          },
          "塩を使うのは、ぬめりのたんぱく質を固めて落としやすくするためだ。水で流すだけでは取り切れない。",
        ],
      },
      {
        h2: "手順1：ウロコを取る",
        body: [
          "ニジマスのウロコは細かく薄いので、意識しないと見落とす。ただし焼くと口に残るので、軽く落としておく。",
          "包丁の背かウロコ取りで、尾から頭へ向かって軽く撫でる。力は要らない。",
          "背びれの際、腹びれの根元、頭の後ろが残りやすい。",
        ],
      },
      {
        h2: "手順2：エラと内臓を出す",
        body: [
          "塩焼きにするなら頭は落とさない。頭を残したまま、エラと内臓を抜く。",
          {
            steps: [
              "エラ蓋を持ち上げ、エラの上下の付け根を切り離して引き抜く。",
              "肛門から胸びれの手前まで、腹を切り開く。",
              "内臓を指でつまんで引き出す。ニジマスの内臓はまとまって取れる。",
              "背骨に沿った黒い血合いを、指の腹か爪でこすり落とす。",
            ],
          },
          { alert: "血合いは膜に包まれていて、水で流すだけでは落ちない。指か爪、あるいは歯ブラシでこそげ取る。ここが残ると焼いたときに苦味と臭みが出る。" },
          "唐揚げやムニエルにするなら頭を落としてもいい。胸びれの後ろから斜めに包丁を入れる。",
        ],
      },
      {
        h2: "手順3：洗って水気を拭く",
        body: [
          "腹の中を流水でよく洗う。血合いの残りがないか、指で確かめる。",
          { alert: "洗ったあと長く水に浸けない。淡水魚を真水に浸けておくと身が水を吸って水っぽくなる。" },
          "キッチンペーパーで表面と腹の中の水気を徹底的に拭き取る。ここを省くと、焼いても揚げても仕上がりがぼやける。",
          "釣り場から自宅までの持ち帰り方は<a href=\"/guide/kanri-tsuriba-mochikaeri/\">管理釣り場の魚の持ち帰り方</a>にまとめてある。淡水魚は潮氷が使えないので、海の魚とは扱いが変わる。",
        ],
      },
      {
        h2: "料理別の下ごしらえ",
        body: [
          {
            list: [
              "塩焼き：頭付きのまま。焼く20〜30分前に塩をふって水気を拭くと身が締まる。ヒレに化粧塩をすると焦げない",
              "ムニエル：三枚おろしか、腹開き。水気を拭いて小麦粉をまぶす",
              "ホイル焼き：頭付き、または頭を落として。腹に香草やきのこを詰める",
              "唐揚げ：頭を落として二等分か、小型ならそのまま。骨まで食べられる",
              "南蛮漬け：頭を落として素揚げ。二度揚げすると骨ごといける",
              "燻製：三枚おろしか腹開きで、塩水に漬けてから乾かす",
            ],
          },
          "ニジマスは骨が柔らかいので、20〜25cmの個体なら三枚におろさずそのまま調理できる料理が多い。手間をかけずに済むのが管理釣り場の魚の良さでもある。",
          "燻製にする場合の塩水の作り方は、<a href=\"/guide/himono/\">干物の作り方</a>の立て塩の項が参考になる。",
        ],
      },
      {
        h2: "小骨の扱い",
        body: [
          "ニジマスには血合い骨（小骨）があるが、細くて柔らかい。",
          {
            list: [
              "塩焼き・唐揚げ：気にしなくてよい。焼けば箸で外せる",
              "ムニエル・ホイル焼き：食べるときに背骨ごと外せば小骨も一緒に取れる",
              "刺身にはしないので、骨抜きで一本ずつ抜く必要はほとんどない",
            ],
          },
          "焼き魚を骨ごときれいに食べるには、背骨に沿って箸で身を開き、頭側から背骨を持ち上げる。ニジマスは骨と身が離れやすいので、この方法がよく効く。",
        ],
      },
      {
        h2: "生では食べない",
        body: [
          { alert: "釣り場のニジマスを刺身で食べるのは避ける。淡水魚には、海水魚のアニサキスとは別の寄生虫のリスクがある。" },
          "サケ・マス類には日本海裂頭条虫が、淡水魚全般には顎口虫や横川吸虫が寄生することがある。市販の生食用サーモンが生で流通しているのは、生食を前提とした管理と冷凍処理を経ているからで、釣り場の魚がそれと同じ状態にあるわけではない。",
          "加熱すれば問題ないので、塩焼き・ムニエル・ホイル焼き・唐揚げのいずれかにする。ニジマスは淡白なので、加熱料理でも十分に美味しい。",
          "詳しくは<a href=\"/guide/kanri-tsuriba-mochikaeri/\">管理釣り場の魚の持ち帰り方</a>にまとめてある。",
        ],
      },
      {
        h2: "保存",
        body: [
          {
            list: [
              "当日：下処理して水気を拭き、ラップで包み冷蔵",
              "翌日まで：同上。淡水魚は身が柔らかいので早めに食べる",
              "数日以上：内臓を抜いて水気を拭き、1尾ずつラップ＋保存袋で冷凍（3〜4週間）",
            ],
          },
          "数が釣れたときは、その日のうちに全部下処理してから冷凍するのがいちばん失敗しない。内臓を残したまま冷凍すると、解凍後に臭みが回る。",
          "ニジマスのレシピは<a href=\"/fish/nijimasu/\">ニジマスのページ</a>、冷凍の詳細は<a href=\"/guide/reitou-hozon/\">釣った魚の冷凍と保存</a>にまとめてある。",
        ],
      },
    ],
  },
  {
    slug: "sanmai-oroshi",
    title: "三枚おろしの基本｜どの魚でも同じ5つの手順",
    date: "2026-08-02",
    description: "三枚おろしの手順を、包丁の入れ方から中骨の外し方まで解説。身が骨に残る・身が崩れるといった失敗の原因と対処、大名おろしとの使い分けまでまとめています。",
    gear: [
      { name: "出刃包丁", q: "出刃包丁 165mm" },
      { name: "骨抜き", q: "骨抜き ステンレス 魚" },
      { name: "砥石", q: "砥石 中砥 1000番" },
      { name: "まな板", q: "まな板 抗菌 大きめ" },
    ],
    lead: "三枚おろしは、魚が違っても手順は変わらない。背から切る、腹から切る、中骨の上を通す。この3つの繰り返しで、上身・下身・中骨の三枚になる。難しく感じるのは手順が多いからではなく、一度で切ろうとするからだ。",
    sections: [
      {
        h2: "うまくいかない原因はほぼ2つ",
        body: [
          "三枚おろしで失敗する原因は、たいていこの2つに絞られる。",
          {
            list: [
              "包丁が切れない：身を切るのではなく押し潰すことになり、断面がぼろぼろになる",
              "一度で切り離そうとする：深く入れすぎて中骨を割ったり、浅すぎて身が骨に残る",
            ],
          },
          "逆にいえば、よく研いだ包丁で浅い切り込みを何度も繰り返せば、それだけでかなりきれいにおろせる。慣れないうちは3〜4回に分けるつもりでいい。",
          "包丁の選び方と研ぎ方は<a href=\"/guide/deba-bocho/\">出刃包丁の選び方と研ぎ方</a>にまとめてある。",
        ],
      },
      {
        h2: "手順1：ウロコ・頭・内臓",
        body: [
          "三枚におろす前に、下処理を済ませておく。ここが雑だと、あとの工程で身が汚れる。",
          {
            steps: [
              "ウロコを尾から頭に向かって落とす。",
              "胸びれの後ろから斜めに包丁を入れ、裏返して同じ角度で入れて頭を落とす。",
              "腹を肛門から切り開き、内臓を出す。",
              "背骨に沿った血合いを掻き出し、流水で洗う。",
              "キッチンペーパーで水気を徹底的に拭く。",
            ],
          },
          { alert: "水気を拭かずに次へ進むと、まな板の上で魚が滑って危ない。仕上がり以前に安全の問題になる。" },
        ],
      },
      {
        h2: "手順2：背から切る",
        body: [
          { svg: "sanmai-junjo" },
          "頭側を右、背を手前にして置く。背びれの少し上に包丁を当て、背骨に向かって浅く切り込みを入れる。",
          {
            steps: [
              "1回目：皮を切る程度の浅さで、頭側から尾まで線を引く。",
              "2回目：同じ線をなぞって少し深くする。",
              "3回目：背骨に当たる感触が出るまで深くする。",
            ],
          },
          "包丁は寝かせすぎず、まな板に対して45度くらいの角度で入れる。骨に当たったらそれ以上押し込まない。",
        ],
      },
      {
        h2: "手順3：腹から切る",
        body: [
          "魚を回して腹を手前にする。今度は腹側から同じ要領で切り込む。",
          "腹骨があるので、背側より抵抗がある。無理に押し切らず、骨の上を滑らせるように包丁を進める。",
          "背と腹の両方から切り込みが入ると、身は中骨の上でだけ繋がった状態になる。",
        ],
      },
      {
        h2: "手順4：中骨の上を通す",
        body: [
          { svg: "sanmai-hocho-kaku" },
          "ここが山場になる。包丁を寝かせ、中骨の上面に沿わせて滑らせる。",
          {
            list: [
              "刃はまな板と平行に近い角度まで寝かせる",
              "骨に当てながら滑らせる。骨から浮くと身に骨が残る",
              "押さずに引く。刃元から切っ先へ、包丁の長さを使う",
              "左手は身を軽く押さえるだけ。引っ張り上げない",
            ],
          },
          "尾の付け根まで通ったら片身が外れる。裏返して、同じ手順をもう一度繰り返す。",
        ],
      },
      {
        h2: "手順5：腹骨と血合い骨",
        body: [
          {
            steps: [
              "腹骨を削ぐ。包丁を寝かせ、骨の下に差し入れて薄く削ぎ取る。身を厚く落とさないよう浅く。",
              "血合い骨を処理する。骨抜きで真上に抜くか、骨の両側に切り込みを入れて細く切り取る。",
            ],
          },
          "骨抜きで抜くときは、斜めに引っ張ると身が裂ける。骨の向きに沿って、まっすぐ引き抜く。",
          "血合い骨は魚によって太さが違う。アジやイワシは細く抜きやすいが、タイやブリは太いので切り取るほうが確実なことが多い。",
        ],
      },
      {
        h2: "大名おろしという選択肢",
        body: [
          "背と腹から切らず、頭側から尾に向かって中骨の上を一気に通すやり方を大名おろしという。",
          {
            list: [
              "利点：速い。手順が少ない",
              "欠点：中骨に身が多く残る。歩留まりが悪い",
              "向くのは：サンマ・イワシ・キスなど、細長くて骨の細い魚",
            ],
          },
          "大名という名前は「もったいない切り方」という皮肉から来ている。数が釣れる小魚をまとめて処理するときには合理的だが、タイやブリのような魚には向かない。",
        ],
      },
      {
        h2: "うまくいかないときは",
        body: [
          {
            list: [
              "中骨に身が残る：包丁が骨から浮いている。骨に当てながら滑らせる",
              "身が崩れる：包丁が切れない。研ぐ",
              "断面がぼろぼろ：押して切っている。引いて切る",
              "皮が破ける：切り込みが浅く、皮だけ残っている",
              "身が骨まで切れてしまう：一度で深く入れすぎ。浅い切り込みを繰り返す",
              "手が滑る：水気が残っている。ペーパーで拭く",
            ],
          },
          "最初の数匹は中骨に身が残って当然だ。残った中骨はアラ汁や潮汁に使えばいいので、練習の失敗も無駄にはならない。",
        ],
      },
      {
        h2: "魚ごとの注意点",
        body: [
          {
            list: [
              "アジ：先にゼイゴを取る。<a href=\"/guide/aji-shitagoshirae/\">アジの下処理</a>",
              "サバ：傷みが速いので手早く。<a href=\"/guide/saba-shitagoshirae-anisakis/\">サバの下処理とアニサキス対策</a>",
              "タイ：ウロコが硬く体高がある。背と腹から別々に。<a href=\"/guide/tai-shitagoshirae/\">タイの下処理</a>",
              "ブリ：大きいので道具とまな板から。<a href=\"/guide/buri-shitagoshirae/\">ブリ・ハマチの下処理</a>",
              "イワシ：包丁を使わず手で開ける。<a href=\"/guide/iwashi-tebiraki/\">イワシの手開き</a>",
            ],
          },
          "手順そのものはどの魚でも同じなので、1種類できるようになれば他の魚にも応用が利く。",
        ],
      },
    ],
  },
  {
    slug: "mebaru-shitagoshirae",
    title: "メバルの下処理｜煮付けのための飾り包丁とエラの抜き方",
    date: "2026-08-02",
    fish: "メバル",
    description: "メバルの下処理を解説。棘への注意、頭を残したままのエラと内臓の抜き方（壺抜き）、煮付け用の飾り包丁の入れ方、鮮度の見分け方までまとめています。",
    gear: [
      { name: "小出刃包丁", q: "出刃包丁 小 105mm" },
      { name: "キッチンバサミ", q: "キッチンバサミ 魚 骨" },
      { name: "ウロコ取り", q: "ウロコ取り 魚 コンパクト" },
      { name: "菜箸", q: "菜箸 ステンレス 長い" },
    ],
    lead: "メバルは煮付けにすると際立つ魚だ。身は淡白で締まり、皮に旨味がある。姿のまま煮ることが多いので、下処理も「頭を残したまま、いかにきれいに中身を抜くか」が中心になる。",
    sections: [
      {
        h2: "棘に注意する",
        body: [
          "メバルは背びれ・腹びれ・尻びれ・エラ蓋に棘がある。カサゴほど太くはないが、細いぶん刺さりやすい。",
          { alert: "掴むときは下あごを持つか、背びれを寝かせて背中側から包み込むように持つ。エラ蓋に指を入れない。" },
          "煮付けにするなら背びれは残すことが多いので、無理に切り落とさなくてよい。ただし調理中と食べるときに刺さらないよう、家族に伝えておく。",
          "根魚の棘の扱いは<a href=\"/guide/kasago-shitagoshirae/\">カサゴの下処理</a>に図解つきでまとめてある。",
        ],
      },
      {
        h2: "鮮度は目で分かる",
        body: [
          "メバルは「目を張る」から来た名前と言われるほど目が大きい。この目が鮮度の指標になる。",
          {
            list: [
              "目が澄んで盛り上がっている：新しい",
              "目が白く濁り、へこんでいる：時間が経っている",
              "エラが鮮紅色：新しい。褐色になっていたら古い",
              "体表にぬめりがあり、張りがある：新しい",
            ],
          },
          "釣った直後は当然どれも新しいが、持ち帰りの温度管理を外すと数時間で目が濁る。潮氷で冷やす意味はここにも出る。持ち帰り方は<a href=\"/guide/mochikaeri/\">釣った魚の持ち帰り方</a>にまとめてある。",
        ],
      },
      {
        h2: "手順1：ウロコを取る",
        body: [
          "メバルのウロコは中くらいの大きさで、比較的落としやすい。ウロコ取りか包丁の背で、尾から頭へ向かって落とす。",
          "姿煮にするなら、頭の周りとヒレの根元まで丁寧に。皮ごと食べる料理なので、取り残しは口に残る。",
          "流水を当てながらやると飛び散らない。",
        ],
      },
      {
        h2: "手順2：エラと内臓を抜く（壺抜き）",
        body: [
          { svg: "mebaru-tsubonuki" },
          "姿のまま煮るときは、腹を大きく開かずに中身を抜く。これを壺抜きという。腹が裂けないので煮崩れしにくく、見た目もよい。",
          {
            steps: [
              "エラ蓋を持ち上げ、エラの上下の付け根を包丁の先で切り離す。",
              "菜箸を2本、口から入れる。1本はエラの内側、もう1本はエラ蓋側から差し込む。",
              "2本の箸でエラを挟み、そのまま内臓ごとゆっくりねじりながら引き出す。",
              "肛門に浅く切り込みを入れておくと、内臓が切れずに抜けやすい。",
            ],
          },
          "うまく抜けないときは無理をせず、腹を肛門から胸びれの手前まで小さく切り開いて出す。煮付けなら多少の切り口は気にならない。",
          { alert: "内臓が途中で切れると腹の中に残る。残ると苦味と臭みが出るので、切れたら指や箸で確実にかき出す。" },
        ],
      },
      {
        h2: "手順3：血合いを落とす",
        body: [
          "背骨に沿った血合いは、腹の中の奥に張り付いている。壺抜きでは手が入りにくいので、細い菜箸か歯ブラシを使う。",
          {
            steps: [
              "流水を腹の中に通す。",
              "歯ブラシか箸の先で、背骨に沿って血合いをこすり落とす。",
              "水が濁らなくなるまで繰り返す。",
              "腹の中まで水気を拭き取る。",
            ],
          },
          "ここを省くと、どれだけ丁寧に煮ても汁が濁って生臭くなる。メバルの下処理で最も味に効く工程がここだ。",
        ],
      },
      {
        h2: "手順4：飾り包丁を入れる",
        body: [
          { svg: "mebaru-kazari" },
          "姿煮にするときは、身の厚い部分に切り込みを入れる。理由は2つある。",
          {
            list: [
              "火の通りを均一にする（中心まで熱が入る）",
              "煮汁の味を身に入れる",
            ],
          },
          "入れ方は、体の中央に斜めの切り込みを1〜2本。深さは中骨に当たる手前まで。両面に入れる。",
          "深く入れすぎると身が割れて煮崩れる。皮と身の厚みの半分程度で止める。",
        ],
      },
      {
        h2: "料理別の使い分け",
        body: [
          {
            list: [
              "煮付け：定番。壺抜き＋飾り包丁。頭ごと煮て頬肉まで食べる",
              "唐揚げ：小型ならまるごと。二度揚げすると骨まで食べられる",
              "塩焼き：頭付きのまま。ヒレに化粧塩をすると焦げない",
              "味噌汁・潮汁：アラから濃い出汁が出る",
              "刺身：25cm以上の良型なら。身が締まって甘みがある",
            ],
          },
          "20cm前後がよく釣れるサイズで、これは煮付けか唐揚げが向く。三枚におろすには小さいので、姿のまま調理するほうが歩留まりもいい。",
          "メバルのレシピは<a href=\"/fish/mebaru/\">メバルのページ</a>にまとめてある。",
        ],
      },
      {
        h2: "保存",
        body: [
          {
            list: [
              "当日〜翌日：下処理して水気を拭き、ラップで包み冷蔵",
              "数日以上：下処理後に1尾ずつラップ＋保存袋で冷凍（3〜4週間）",
            ],
          },
          "白身で傷みにくいほうだが、内臓を抜いていない状態では話が別になる。釣った日に壺抜きまで済ませておくと、あとが楽だ。",
          "冷凍の詳細は<a href=\"/guide/reitou-hozon/\">釣った魚の冷凍と保存</a>を参照してほしい。",
        ],
      },
    ],
  },
  {
    slug: "deba-bocho",
    title: "出刃包丁の選び方と研ぎ方｜魚をさばく道具の基本",
    date: "2026-08-02",
    description: "出刃包丁の選び方と研ぎ方を解説。刃渡りのサイズ選び、片刃と両刃の違い、鋼とステンレスの使い分け、砥石での研ぎ方と返りの確認、錆びさせない手入れまでまとめています。",
    gear: [
      { name: "小出刃包丁", q: "出刃包丁 小 105mm" },
      { name: "出刃包丁（標準）", q: "出刃包丁 165mm" },
      { name: "砥石（中砥）", q: "砥石 中砥 1000番" },
      { name: "砥石（仕上げ）", q: "砥石 仕上げ 3000番" },
    ],
    lead: "魚をさばくとき、腕前より先に結果を左右するのが包丁の切れ味だ。切れない包丁は身を切らずに押し潰すので、どれだけ丁寧にやっても断面がぼろぼろになる。逆に、よく研げた包丁があれば手順はあとからついてくる。",
    sections: [
      {
        h2: "なぜ出刃が要るのか",
        body: [
          "三徳包丁や牛刀でも魚はさばける。ただし骨に当たったときに差が出る。",
          {
            list: [
              "出刃は刃が厚く重い。重さで骨を断てる",
              "三徳・牛刀は刃が薄い。骨に当てると刃が欠ける",
            ],
          },
          "アジやイワシのような骨の柔らかい小魚なら三徳でも足りる。頭を落とす、カマを外す、といった工程が入るなら出刃が要る。",
          "「切れ味」ではなく「骨を断つための重さと厚み」が出刃の本質だと考えるといい。",
        ],
      },
      {
        h2: "サイズの選び方",
        body: [
          { svg: "deba-size" },
          "刃渡りは、狙う魚の大きさで決める。",
          {
            list: [
              "105〜120mm（小出刃）：アジ・イワシ・キス・メバルなど30cm以下。取り回しがよく最初の一本に向く",
              "150〜165mm（標準）：サバ・タイ・スズキなど50cmまで。家庭で一本だけ持つならこのサイズ",
              "180mm以上：ブリ・大型青物。それ以外では重くて扱いにくい",
            ],
          },
          "迷ったら150〜165mmを選ぶ。小型魚にはやや大きいが、使えないことはない。逆に小出刃で大型魚をさばくのは危険で、骨に負けて刃が欠ける。",
          { alert: "刃渡りが長いほど良いわけではない。まな板からはみ出す長さの包丁は、家庭では扱いにくいだけになる。" },
        ],
      },
      {
        h2: "片刃と両刃",
        body: [
          "出刃は基本的に片刃で、右利き用は右側にだけ刃がついている。",
          {
            list: [
              "片刃：骨に沿わせやすく、身を薄く削ぎやすい。和包丁の標準",
              "両刃：まっすぐ切り下ろしやすい。洋包丁と同じ感覚で使える",
            ],
          },
          { alert: "左利きの人が右利き用の片刃を使うと、刃が斜めに入って身が曲がる。左利き用を選ぶか、両刃を選ぶ。" },
          "片刃は研ぎ方も片面が中心になる（後述）。両刃より研ぎやすい面もあるので、最初から片刃で覚えてしまってよい。",
        ],
      },
      {
        h2: "鋼とステンレス",
        body: [
          {
            list: [
              "鋼（はがね）：よく切れて研ぎやすい。ただし錆びる。使うたびに拭いて乾かす必要がある",
              "ステンレス：錆びにくく手入れが楽。鋼よりわずかに切れ味が落ちるが、家庭では十分",
            ],
          },
          "釣った魚を月に数回さばく程度なら、ステンレスをすすめたい。鋼は放置すると一晩で錆が出る。手入れを楽しめる人以外には向かない。",
          "「ステンレスは研げない」と言われることがあるが、誤解だ。普通に砥石で研げる。",
        ],
      },
      {
        h2: "研ぎ方",
        body: [
          { svg: "deba-togi" },
          "砥石は中砥（1000番前後）が1つあれば足りる。仕上げ砥（3000番以上）は切れ味を上げたい人向けで、必須ではない。",
          {
            steps: [
              "砥石を10〜15分ほど水に浸ける。気泡が出なくなったら準備完了。",
              "濡らした布巾の上に砥石を置いて固定する。",
              "刃を砥石に当て、刃の裏側に10円玉2枚分ほどの隙間ができる角度（約15度）を保つ。",
              "刃元から切っ先まで、区間に分けて研ぐ。押すときに力を入れ、引くときは力を抜く。",
              "反対側（裏）に「返り」が出るまで研ぐ。",
              "返りが出たら裏返し、裏面を砥石に密着させて2〜3回撫でて返りを落とす。",
            ],
          },
          "片刃の場合、研ぐのは表面が中心で、裏面は返りを取るだけだ。裏を大きく研ぐと刃の形が崩れる。",
        ],
      },
      {
        h2: "研げたかどうかの確かめ方",
        body: [
          "返りとは、研いだ結果として刃先の反対側にめくれ上がる金属のバリのことだ。これが出ていれば、刃先までしっかり研げている。",
          {
            list: [
              "指の腹を刃先に対して直角に、刃と反対方向へ軽く滑らせる。ざらつきがあれば返り",
              "刃を蛍光灯にかざす。刃先が白く光る部分は、まだ研げていない",
              "新聞紙を切ってみる。引っかからずにすっと切れれば十分",
            ],
          },
          { alert: "返りの確認で、刃に沿って指を滑らせないこと。切る。刃と直角の方向に動かす。" },
          "研ぎたては刃先が荒れているので、新聞紙などを数回切って落ち着かせるとよい。",
        ],
      },
      {
        h2: "手入れ",
        body: [
          {
            list: [
              "使ったらすぐ洗う。魚の血や内臓は錆の原因になる",
              "洗ったら必ず拭いて乾かす。濡れたまま置かない",
              "食洗機に入れない。柄が傷み、刃も傷む",
              "まな板は木か樹脂。ガラスや陶器の上で切ると刃が一発で潰れる",
              "保管は乾いた場所へ。シンク下の湿気は避ける",
            ],
          },
          "研ぐ頻度は使用回数による。魚を数匹さばくごとに軽く研ぐくらいで、切れ味を保てる。切れなくなってから研ぐより、切れているうちに軽く研ぐほうが早く終わる。",
        ],
      },
      {
        h2: "まとめ",
        body: [
          "最初の一本を選ぶなら、150〜165mmのステンレス片刃（左利きなら左利き用）と、1000番の砥石。これで釣れる魚のほとんどに対応できる。",
          "そして何より、切れる状態を保つこと。三枚おろしがうまくいかない原因の半分は、腕ではなく刃だ。おろし方そのものは<a href=\"/guide/sanmai-oroshi/\">三枚おろしの基本</a>にまとめてある。",
        ],
      },
    ],
  },
  {
    slug: "sashimi-kirikata",
    title: "刺身の切り方｜平造りとそぎ造りの使い分け",
    date: "2026-08-02",
    description: "刺身の切り方を解説。平造り・そぎ造り・薄造りの使い分け、包丁を引いて切る理由、柵の置き方、厚みの目安、盛り付けの考え方までまとめています。",
    gear: [
      { name: "柳刃包丁", q: "柳刃包丁 210mm" },
      { name: "砥石（中砥）", q: "砥石 中砥 1000番" },
      { name: "まな板", q: "まな板 抗菌 大きめ" },
      { name: "刺身皿", q: "刺身皿 和食器" },
    ],
    lead: "刺身は切り方で味が変わる。同じ魚、同じ鮮度でも、断面が潰れているかどうかで舌触りがまるで違う。押さずに引く。これだけ守れば、家庭でも十分に美味しい刺身になる。",
    sections: [
      {
        h2: "押して切らない",
        body: [
          "刺身の切り方でいちばん大事なのはここだ。包丁を前後に往復させたり、上から押し付けて切ってはいけない。",
          "魚の身は繊維が繊細で、押されると潰れる。潰れた断面は光を乱反射してくすみ、舌触りもざらつく。",
          {
            steps: [
              "包丁の刃元を身に当てる。",
              "手前に引きながら、刃全体を使って切り下ろす。",
              "一度で切り離す。往復させない。",
            ],
          },
          "柳刃包丁が長いのはこのためだ。刃渡りが長いほど、一度の引きで切り切れる。柳刃がなければ、家にある包丁でいちばん長いものを使えばよい。三徳でも、押さずに引けばかなり違う。",
          "そして前提として、包丁が切れること。研ぎ方は<a href=\"/guide/deba-bocho/\">出刃包丁の選び方と研ぎ方</a>にまとめてある。",
        ],
      },
      {
        h2: "柵の置き方",
        body: [
          "三枚におろして皮を引いた身（柵）を、まな板の手前側に置く。",
          {
            list: [
              "厚みのある側を奥、薄い側を手前にする",
              "皮があった面を上に、血合いが手前側に来るよう置くと切りやすい",
              "まな板の右端に寄せると、切った身をそのまま右へ送れる",
            ],
          },
          "身を切るたびに包丁の腹で右へ送り、並べていく。こうすると盛り付けのときに順番が崩れない。",
          "皮の引き方は<a href=\"/guide/suzuki-shitagoshirae/\">スズキの下処理</a>に図解つきでまとめてある。",
        ],
      },
      {
        h2: "平造り",
        body: [
          { svg: "sashimi-hira" },
          "いちばん基本の切り方で、厚みのある身に向く。アジ、ブリ、ハマチ、サバなど。",
          {
            steps: [
              "包丁を身に対して垂直に立てる。",
              "刃元を当て、手前に引きながら切り下ろす。",
              "切れたら包丁の腹で身を右へ送る。",
            ],
          },
          "厚さの目安は7〜10mm。脂の乗った魚は厚めに、淡白な白身は薄めにすると食べやすい。",
          "身の繊維に対して直角に切ると、口の中でほろりとほどける。柵の向きを確認してから切りはじめる。",
        ],
      },
      {
        h2: "そぎ造り",
        body: [
          { svg: "sashimi-sogi" },
          "包丁を寝かせて、そぐように切る。身の薄い魚や、身の締まった白身に向く。タイ、スズキ、ヒラメなど。",
          {
            steps: [
              "包丁を左に傾け、30〜45度ほど寝かせる。",
              "刃元を当て、身の左端から手前に引きながらそぐ。",
              "切り離したら、左手で身を返して器に並べる。",
            ],
          },
          "断面が広く取れるので、薄くても食べごたえが出る。歯ごたえの強い白身は、この切り方のほうが噛み切りやすい。",
          "極端に薄くそいで皿の模様が透けるようにしたものが薄造りで、フグやヒラメで使われる。よく切れる包丁でないと身が裂ける。",
        ],
      },
      {
        h2: "魚別の向き",
        body: [
          {
            list: [
              "アジ：平造り。小型は皮を手で引いてから",
              "サバ：しめサバにして平造り。生食は冷凍が前提",
              "タイ：そぎ造り。皮つきなら松皮造り",
              "スズキ：そぎ造り。夏場は氷水で締める洗いも",
              "ブリ・ハマチ：平造り。脂が多いので厚め",
              "ニジマスなど淡水魚：生食にしない",
            ],
          },
          "脂の多い魚は厚く、締まった白身は薄く。これだけ覚えておけば大きく外さない。",
        ],
      },
      {
        h2: "盛り付け",
        body: [
          "難しく考えなくてよい。押さえるのは3点だけだ。",
          {
            list: [
              "奥を高く、手前を低く。立体感が出る",
              "左から右へ、少しずつ重ねて並べる",
              "大葉や大根のツマを奥に置き、身をもたせかける",
            ],
          },
          "皿は身より一回り大きいものを選び、余白を残す。詰め込むと見た目が重くなる。",
          "切った刺身は時間とともに水分が出るので、食べる直前に切って盛るのがいちばんうまい。",
        ],
      },
      {
        h2: "生食の前提",
        body: [
          { alert: "刺身にするかどうかは、切り方の前に鮮度と寄生虫のリスクで決まる。" },
          {
            list: [
              "釣ったその日に内臓を抜き、冷やして持ち帰った魚であること",
              "アニサキスのリスクを理解していること（サバ・アジ・イワシ・ブリなど）",
              "淡水魚は生食しない",
            ],
          },
          "アニサキスの見分け方と対処は<a href=\"/guide/saba-shitagoshirae-anisakis/\">サバの下処理とアニサキス対策</a>、淡水魚のリスクは<a href=\"/guide/kanri-tsuriba-mochikaeri/\">管理釣り場の魚の持ち帰り方</a>にまとめてある。",
          "迷ったら加熱する。加熱料理でも釣った魚は十分に美味しい。",
        ],
      },
    ],
  },
];

if (typeof module !== "undefined") {
  module.exports = { FISH_LIST, PLACES, TOOLS, SEASONINGS_LIST, AMAZON_BASE, AMAZON_TAG, ADSENSE_ENABLED, ADSENSE_CLIENT, RECIPES, FISHING_LOGS, GUIDES };
}
